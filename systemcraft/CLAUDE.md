...
| `templates/` | public | Artifact templates: PRD, ADR, failure-UX spec + model card, eval plan, ops/economics model + incident runbook, the handoff return note — plus ratified law surfaces: status vocabulary, close-digest format |
...
7. **Handoffs cross typed artifacts and references, never reasoning** *(Productcraft build map #271, ratified 2026-09-11)*. Another studio hands Systemcraft a typed brief plus frozen, hashed copies of its artifacts under [the handoff contract](../productcraft/templates/handoff-contract.md); the Design Strategist issues the crossing's typed state at Open; the seats design from the sender's graded evidence and never re-run its discovery or re-litigate its product decision; the return after Gate 2 is a typed note plus frozen copies, per [templates/handoff-return.md](templates/handoff-return.md). Neither studio reads or writes inside the other's ledger: Systemcraft writes only `originates_from` and `supersedes_external`, and ids are the only thing that cross.
