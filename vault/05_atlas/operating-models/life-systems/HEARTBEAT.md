---
type: operating-model
artifact: HEARTBEAT
domain: [life-systems]
status: draft
last_interviewed: 2026-04-21
created: 2026-04-18
review-date: null
ai-context: "Operating rhythms for personal systems — finance, health, learning, tasks, time, career transition. Populated by the work-operating-model skill. Consumed by meeting-defender and daily-driver."
---

# HEARTBEAT — Life Systems

## Daily Rhythm

Weekday (Boston, post-move):

| Time | Block | Notes |
|---|---|---|
| 05:15–05:30 | Wake | Later than the legacy 4:45 — shifted with the Boston move |
| 05:30–06:30 | **Sacred first hour** | Coffee, breakfast prep, green juice, vitamins, decompress. YouTube AI news, podcast, or creative project |
| 06:50 | Leave for gym | 10-minute walk |
| 07:00–08:00 | Gym | 1 hr workout |
| 08:15–08:30 | Home | Eat breakfast |
| 08:45–09:00 | Workday start | |
| 09:00–14:00 | **Deep work (45%)** | Block work primarily; protected from meetings where possible |
| 11:30–12:30 | Lunch | Usually eat at desk + watch YouTube to learn AI news/tools/workflows |
| 14:00–15:00 | Decompress | Walk or sit on couch — away from computer |
| 15:00–17:15 | **Comms/exec (35%)** | Finish in-flight work, Slack triage, plan tomorrow, Obsidian list grooming |
| 17:15–~21:00 | Evening | Dinner with girlfriend, relax, optional learning/research/creative |
| 21:00–22:00 | Bed | Target sleep window |

**Life-systems windows around the workday** (the 45/35/20 split is a work-time rule — see [the-block/CLAUDE.md](../../../../the-block/CLAUDE.md); life-systems needs the *inverse* — when life-systems time actually happens):
- Pre-work 05:30–08:45 — sacred first hour + gym + breakfast (life-systems anchor block)
- Lunch 11:30–12:30 — learning (AI news / tools / workflows on YouTube)
- Decompress 14:00–15:00 — walk or couch, no computer (protected life-systems slot)
- Evening 17:15–21:00 — dinner w/ girlfriend, relaxed learning/creative woven in
- Work displaces life-systems during 09:00–14:00 and 15:00–17:15

**Weekend**: No gym — body needs recovery. Walk for movement. Focus is recovery, relaxation, quality time with girlfriend/friends/family, and personal creative + agentic workflow projects.

## Weekly Cadence

**Calendar (sean.winslow28@gmail.com)**: Essentially empty — no recurring life-systems calendar events. Gmail *inbox* gets heavy notification noise from subscribed sites and past purchases, but those are emails, not calendar events.

**Off-calendar weekly rituals (current + aspirational)**:
- `/daily-driver` weekly review — currently work-focused, want to extend to life-systems
- Wants in the weekly review:
  - Financial goals review (savings, debt paydown progress)
  - Money-making research scan (stocks, crypto, side freelance, agentic side projects)
  - "Things I learned this week" log — capture tools, workflows, news for future reference
  - Quick-capture inbox sweep — drop-anything-into-Obsidian pattern, with agents able to reference it in future conversations
- Example seed idea: [idea-autoresearch-agentic-polymarket-crypto](../../../40_knowledge/concepts/idea-autoresearch-agentic-polymarket-crypto.md) — candidate for a Perplexity-API research agent that produces an overnight report

## Monthly / Quarterly

**Monthly money cycle (every month, fixed)**:
- **1st** — Rent due (paid via Bilt)
- **4th** — Chase bank statement closes
- **7th** — Chase credit card payment due
- **10th** — Chase credit card statement closes
- **12th** — Bilt credit card payment due

**Quarterly**: No estimated tax payments — W-2 at The Block since November 2025. Taxes withheld via Rippling.

**Subscription audit**: Last full audit 2026-02-27 → [financial-review-2026-02-27.md](../../../50_sources/finance/financial-review-2026-02-27.md). Heavy cuts already applied. Re-audit pending — Sean flagged in April 2026 as due soon.

## Seasonal Shifts

**Boston winter (vs NYC)**:
- Gym motivation drops but attendance holds — 10-minute walk is short enough to push through
- Only extreme conditions (severe storm, gym closure) break the streak
- Heating on **Eco mode** keeps electric bill steady — do not let it run unchecked
- Holidays = heavier travel + events, but morning routine has held

**Summer (TBD — first Boston summer)**:
- Window-unit AC — electric bill impact unknown, watch closely

**Year-round**: No formal seasonal financial cycle (no quarterly tax, no bonus cycle yet captured).

## Sacred Blocks

1. **05:30–06:30 — Sacred first hour**: coffee, breakfast prep, green juice, vitamins, learning/decompress. Only violators: sickness, unusually heavy workday requiring early start, gym injury recovery.
2. **07:00–08:00 — Gym**: held ~6 days/week. Skipped only for travel, illness, or extreme weather.
3. **09:00–14:00 — Deep work**: protected from meetings whenever possible.
4. **14:00–15:00 — Decompress**: away from computer. Walk or couch.
5. **Evening with girlfriend**: dinner + decompression. Creative/learning work is layered in only when relaxed.

## Email Split

- **sean.winslow28@gmail.com**: ~100% of life-systems traffic — personal, financial, subscriptions
- **swinslow@theblock.co**: only life-systems-adjacent item is Rippling pay stub notifications. Redundant with bank statements — probably not load-bearing for personal finance tracking, but Sean hasn't firmly decided (flag open)
