# Knowledge Lint Report — 2026-08-23

_1311 issues found (1079 structural, 232 semantic)._

_302 finding(s) withheld from this tracked report (SOUL-derived, or about a file in a gitignored subtree). They remain counted below; full detail is in `vault/health/private/2026-08-23-private-findings.md` (local-only, gitignored). Backfilled 2026-09-03._
...
- **contradiction** (T2): `knowledge/concepts/agent-health.md` — These articles define agent health as a metric of operational continuity where 'running processes' can be healthy while producing empty/stale outputs (latent failure). This contradicts the concept `agent-fleet-observability-dashboard` which establishes a strict 'freshness invariant' requiring that a synthesizer must never read an index older than the flush it consumed, implying that stale data is a systemic error condition rather than just a latent failure mode. Furthermore, `agent-health-monitoring` argues that silent failures are more dangerous than loud ones and require explicit instrumentation to detect semantic decay, whereas `agent-health.md` suggests that process completion (even with bad output) is the primary metric of health, creating a tension between 'process health' and 'semantic integrity'.
- **contradiction** (T2): `knowledge/concepts/curation-as-the-new-scarcity.md` — `curation-as-the-new-scarcity` posits that in an environment of abundance, the primary economic value shifts to selection/filtering, making intermediaries validators. `abstraction-layer-shift` argues that as tools lower friction, the value proposition shifts from tool mastery to prompt precision (intent engineering). While not a direct logical contradiction, they present conflicting strategic imperatives: one suggests building a curation/validator role is the high-value play, while the other suggests the high-value play is in the *generation* mechanism (intent/prompt) itself. If intent engineering becomes commoditized (lower friction), curation becomes the moat; if curation becomes commoditized, intent becomes the moat. The corpus does not resolve which layer currently holds the scarcity premium.
- **contradiction** (T2): `knowledge/concepts/complexity-override.md` — `complexity-override` describes relaxing rigid procedural constraints to manage high-entropy environments, shifting discipline from following a path to managing experimental bounds. `capability-aware-scheduling` describes decoupling matching from claiming to allow dynamic negotiation and revalidation against policy constraints. The contradiction lies in the treatment of 'constraints': one advocates for relaxing them to enable exploration (complexity), while the other advocates for strict re-validation against hard requirements before execution (scheduling). In a unified fleet, an agent facing high entropy might be instructed to override constraints (per `complexity-override`) but simultaneously blocked by the scheduler's requirement to validate against those same constraints (per `capability-aware-scheduling`).
- **contradiction** (T2): `knowledge/concepts/eval-vocabulary-as-control-mechanism.md` — Semantic redundancy/near-identity. Both articles define the exact same mechanism: 'evaluation metrics replace traditional Product Requirements Documents (PRDs) as the primary artifact/interface for defining success.' The second article adds that this is operationalized via portable MCP servers, but the core claim and definition are identical, suggesting one should be consolidated or differentiated by scope (e.g., one as theory, one as implementation).
- **contradiction** (T2): `knowledge/concepts/legibility-debt-as-a-supervision-failure-mode.md` — Semantic overlap on 'decay.' `legibility-debt` describes a specific failure mode where structural validity masks semantic decay. `memory-rot` describes the general degradation of semantic value in stored artifacts over time due to lack of pruning. While not strictly contradictory, they describe the same phenomenon (semantic loss despite apparent health) from different angles (supervision failure vs. lifecycle management). They should be linked or merged to avoid treating 'legibility debt' as a distinct mechanism rather than a symptom of unmanaged memory rot.
- **contradiction** (T2): `knowledge/concepts/operational-uptime-vs-cognitive-utility-tension.md` — Semantic redundancy. Both articles describe the tension between operational health/metrics (uptime, process count, visibility) and actual semantic value/utility delivered to the user. The first frames it as a 'tension' where agents execute tasks but produce low-fidelity outputs. The second frames it as a 'misalignment' where systems appear healthy but value generation stagnates. These are effectively the same concept described with slightly different terminology.
- **contradiction** (T2): `knowledge/concepts/silent-decay-in-strategic-pipelines.md` — These two concepts describe the same phenomenon (operational success masking strategic failure) using different terminology. 'Silent decay' focuses on the systemic output (zero meaningful strategic output), while 'Illusion of health' focuses on the metric visibility (successful execution states). They are semantically redundant rather than contradictory, but represent a duplication of insight that could be consolidated.
- **contradiction** (T2): `knowledge/concepts/the-taste-throughput-trade-off-in-agentic-synthesis.md` — Both concepts identify the same hard limit on automation: human cognitive load in curation/evaluation. 'Taste-throughput' frames it as a structural tension between speed and aesthetic fidelity, while 'Supervision fatigue' frames it as the point where monitoring cost exceeds output value. They are complementary descriptions of the same bottleneck.
- **contradiction** (T2): `knowledge/concepts/the-verification-governance-inversion.md` — These two entries define the exact same concept: the misalignment where operational metrics (uptime, logs) are optimized over semantic value/quality. One is a general definition, the other applies it specifically to agentic workflows. They are redundant.
- **contradiction** (T2): `knowledge/concepts/the-homogenization-trap-in-agentic-creative-workflows.md` — 'Homogenization trap' describes the mechanism (optimization for statistical coherence erasing voice), while 'Slop as a trust deficit' describes the outcome (structural outcome of verification lagging production). They are causally linked but distinct; however, they overlap significantly in their diagnosis of why automated creative output fails to build trust.
...
- **broken-wikilink** (T1): `/Users/seanwinslow/Code-Brain/code-brain/vault/knowledge/connections/the-automation-paradox-in-personal-knowledge-infrastructure.md` — Creative Continuity Under Degradation
...
- **soul-tier-a-conflict** (T2): `knowledge/concepts/aesthetic-standardization-as-a-supervisory-mechanism.md` — detail withheld from this tracked report (SOUL-derived); see the local-only sidecar named in the header
...
- **soul-tier-a-conflict** (T2): `knowledge/concepts/corroboration-depth-as-a-gradient-signal.md` — detail withheld from this tracked report (SOUL-derived); see the local-only sidecar named in the header
...
- **soul-tier-a-conflict** (T2): `knowledge/concepts/substack-drafter-agent.md` — detail withheld from this tracked report (SOUL-derived); see the local-only sidecar named in the header
...
- **soul-tier-a-conflict** (T2): `knowledge/concepts/tacit-knowledge-erosion-vs-automation-scale.md` — detail withheld from this tracked report (SOUL-derived); see the local-only sidecar named in the header
