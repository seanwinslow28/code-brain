# Fleet Status — 2026-06-02

**Generated:** 2026-06-02 08:45 by Meta-Agent
**Active agents:** 7 of 12 | **Disabled:** 5

## Active Agent Health

### vault-indexer (2:00 AM daily, Mac Mini, $0.00/run)
- **Status:** healthy
- **Last run:** 2026-06-02T02:00:13
- **Details:** status=success · 6.7h ago · notes='chunks=139, embeddings=139, errors=0'

### vault-synthesizer (2:30 AM daily, MBP (when awake), $0.00/run)
- **Status:** healthy
- **Last run:** 2026-06-02T03:15:31
- **Details:** status=success · 5.5h ago · notes='concepts=2 connections=1 rejected=4 edges=2'

### deep-researcher (2:45 AM daily, Mac Mini, $0.00/run)
- **Status:** healthy
- **Last run:** 2026-06-02T02:45:00
- **Details:** status=empty-queue · mode=queue · 6.0h ago · notes='no unchecked items'

### daily-driver morning (8:45 AM daily, Claude API, ~$0.40/run)
- **Status:** healthy
- **Last run:** 2026-06-02T08:35:08
- **Details:** status=success · mode=morning · 0.2h ago · cost=$0.6848 · notes='Morning planning complete for Tuesday 2026-06-02.  **Daily note created:** `v...'
- **Daily note exists:** Yes (`/Users/seanwinslow/Code-Brain/code-brain/vault/10_timeline/daily/2026-06-02.md`)

### knowledge-lint (Sunday 22:00, Mac Mini / MBP, $0.00/run)
- **Status:** healthy
- **Last run:** 2026-05-31T22:01:55
- **Details:** status=success · 34.7h ago · notes='tier1=324 tier2=40'

### session-end-flush (hook-triggered, Mac Mini / MBP, $0.00/run)
- **Status:** healthy
- **Last run:** 2026-06-02T08:35:08
- **Details:** status=recursion-guard · 0.2h ago · notes='0 msgs'

### meta-agent (8:35 AM daily, local, $0.00/run)
- **Status:** healthy
- **Last run:** 2026-06-02T08:45:04.920580
- **Details:** Generating this report now

## Domain-Aware Insights

_Generated by gemma4:e4b on Mac Mini against schedule-recommendations for creative-studio, life-systems, and job-hunt-2026._

### Aligned with Protect / Automate

- Core vault maintenance agents (indexer/synthesizer) ran successfully, advancing the SSoT architecture.
- The Daily Driver executed morning planning and generated today's note, maintaining process hygiene.
- Background agents (knowledge-lint) completed required synchronization tasks on schedule.

### Misaligned or Touching Decline

- Alienware and ComfyUI are OFFLINE, blocking multi-machine sync and creative pipeline testing.
- The snapshot confirms routine maintenance but lacks evidence of high-leverage, active automations (e.g., finance checks, job aggregation).
- No indication of MCP access troubleshooting, leaving core cross-domain automation reliant on manual intervention.

### Suggestions

- Prioritize resolving the Mac Mini/MBP/Alienware connectivity mesh to enable full agent reach.
- Schedule the next Monthly Finance Check (15th pull) to build capability in financial automation.
- Initiate the formal agent audit and restructure pass, migrating mission-critical agents off flaky machine dependencies.


## Infrastructure

| Machine | Endpoint | Status |
|---------|----------|--------|
| Mac Mini | http://192.168.68.200:11434 | Online |
| Alienware | http://192.168.68.201:11434 | OFFLINE |
| ComfyUI | http://192.168.68.201:8188 | OFFLINE |

## Disabled Agents Reminder

5 agents disabled per AUDIT-2026-04-09-agent-downsizing.md:
- process-inbox, daily-driver evening/weekly, pr-digest, sprint-health
- **Root causes:** CLIConnectionError in SDK transport, MCP servers unavailable in headless mode
- **Do NOT re-enable** without Sean's explicit approval and fixing the underlying SDK bug

## Cost Projection

- vault-indexer: $0.00/month (local)
- vault-synthesizer: $0.00/month (local)
- deep-researcher: $0.00/month (local)
- daily-driver morning: ~$12.00/month
- knowledge-lint: $0.00/month (local)
- session-end-flush: $0.00/month (local)
- meta-agent: $0.00/month (local)
- **Total active fleet:** ~$12.00/month
