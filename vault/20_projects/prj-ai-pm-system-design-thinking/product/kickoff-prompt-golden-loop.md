...
**This project is a learning vehicle first and a portfolio proof second — and it only works as proof because it's honest.** Sean is learning system design thinking by *doing*; every planning move below is deliberately one of the moves his curriculum teaches. Do not shortcut the learning steps to reach outputs faster: **Sean takes every decision and his reasoning goes in the log — but you teach the move first, and draft alongside him.**

> **⚠ TEACHING MODE, 2026-08-17 — supersedes the "Sean predicts first" rule wherever it appears in this file.** The original instruction had Sean predict and draft cold before seeing your work. Sean's correction: *"Change it up so that Fable 5 guides me and explains everything instead of asking me to figure it out based on the teachings. This is part of the teachings as well."* The evidence agrees — worked examples precede faded scaffolding, and discovery-style prompting is counterproductive for material someone is new to. **The full teaching contract, and the rules for fading the scaffolding, live in `kickoff-prompt-phase-c-wayfinder.md` under "TEACHING MODE." Read it and apply it here too.** Prediction-first is not abolished; it is repositioned to material Sean already has the vocabulary for — inside the curriculum modules, where a lesson precedes the exercise.
...
2. **The seed artifact already exists in plan**: the curriculum's capstone — golden dataset + 5-check binary rubric + one champion/challenger round on Sean's real job-feed agent. The build *continues* that artifact; it never forks from it. *(This was "M7's capstone" before the 2026-08-17 rebuild. The live curriculum has five modules; it is now **M5's** capstone and lands week 6–7.)*
...
- **Systems map before PRD.** No requirements until: CLD of the product's intended loops (including its own data flywheel and any degenerate-loop risk), leverage-point analysis, stakeholder-incentive map, and a pre-mortem that finds failure *loops* (not events) and ends in a disposition package (ship/no-ship thresholds, kill criteria, rollback triggers). **Taught and co-drafted** per TEACHING MODE: you explain each technique and draft the first one alongside him; he takes every call and his reasoning goes in the log.
...
**Phase D — Domain & product definition.** `domain-modeling` (Matt Pocock) to pin the ubiquitous language (trace, failure, golden item, rubric check, champion, challenger, holdout, round, promotion, decision record — this vocabulary IS the domain language, on purpose — it comes from the retired M7 lesson, which survives as the vocabulary source even though the live curriculum has five modules and no M7); then `prd-generator` for the PRD and `intent-engineering` for the agentic pieces' intent specs. The **systems map precedes the PRD** (non-negotiable above) — **taught and co-drafted, Sean owns the calls.**
