...
## Addendum — demand-side evidence (Gemini DR run, 2026-08-17, $2.80)

Full report: [../research/2026-08-17-what-do-first-person-practitioner-accounts-team-postmortems.md](../../research/2026-08-17-what-do-first-person-practitioner-accounts-team-postmortems.md) *(vault/20_projects/research/)*. Tier audit: 10% academic / 5% primary / 67% trade / 16% forum — expected for a practitioner-practice question; named accounts weighted, generic trade advice treated as leads per house rule.

What it adds to the verdict, area by area:

- **Holdout discipline is preached and abandoned.** The report's own operational-reality finding: teams under delivery pressure "iterate directly against their entire golden dataset," with the documented offline-online pattern of CI scores inflating to 90–95% while production sits at 50–55% (trade-sourced figures — treat as directional, not citable-precise). Named anchors: OpenAI's 2025 sycophancy rollback postmortem (an eval-feedback-loop failure), the 2024 GPT-4 Turbo silent-regression wave that static overfit suites missed, and Pinterest's offline-metrics-up/online-flat engineering accounts (classic recsys ML, adjacent rather than LLM-native — use with that label).
- **The vocabulary already exists.** One-change-at-a-time is "universally cited" and teams speak champion/challenger, ablation, shadow mode natively — Golden Loop's language will be recognized, not taught. Best named evidence: SWE-agent's ablations (harness config moved pass rates 8.5–13 points vs 2.5–5 for model swaps) and Spotify's Honk ("one change at a time" as explicit trust-architecture doctrine).
- **Decision records are law in finance and folklore everywhere else.** SR 11-7 / SR 26-2 model-risk regimes make immutable sign-off artifacts mandatory in banking; outside regulated sectors the documented norm is Slack screenshots. One rescue account (an enterprise whose eval strategy was "a notebook of 30 examples and a Slack channel"; a CI decision-gate cut hallucinated citations 71%) is effectively Golden Loop's user story told by a consultancy — C-tier source, treat as a lead.
- **The PM-blocked-by-engineering pattern is survey-backed.** A 2026 Deloitte survey of 3,200+ senior leaders finds the enterprise AI bottleneck is governance readiness, not engineering capability; practitioner accounts document PMs owning quality definition while lacking self-serve control of iteration, spawning shadow eval stacks.
- **The report's own thinnest-evidence admission matters for our honesty rules:** public first-person confessions of overfitting to a *proprietary* eval set are rare — companies guard them — so that pain is established indirectly (advisory warnings, adjacent incidents, anonymized rescues). The case study must not claim a corpus of public confessions exists; the mock test (D6) remains the primary demand check.
