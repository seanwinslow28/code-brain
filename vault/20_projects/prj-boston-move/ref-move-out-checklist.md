---
type: reference
domain:
  - life-systems
status: active
context: boston-move
ai-context: "Comprehensive move-out checklist for NYC→Boston relocation, March 21 2026."
created: 2026-03-09
related:
  - "[[prj-boston-move]]"
---

# Move-Out Checklist: NYC → Boston

> **Move-out date:** March 20–21, 2026
> **Lease end:** March 25, 2026
> **New address:** 115 Elm St, Apt 1, Charlestown, MA 02129

---

## Timeline View

### This Week (Mar 9–14) — Do Now
- [ ] Cancel Verizon wifi service
- [ ] Call/email Urby management — see [[#Questions for Urby (One Call)]]
- [ ] Fill out and email Notice to Vacate PDF to Urby
- [ ] Submit forwarding address to Urby (115 Elm St, Apt 1, Charlestown, MA 02129)
- [ ] Link Moved.com account through Urby Resident Portal
- [ ] Pick up 90-count prescription from Rosebank pharmacy (if not already done)
- [ ] Notify manager about March 20 move day (available but no meetings 1–4 PM)
- [ ] Start packing non-essentials

### Move Week (Mar 15–21) — The Move
- [ ] Deep clean apartment: oven, microwave, fridge, general wipe-down (finish by Mar 19)
- [ ] Confirm Hertz reservation — L5151252687 (by Mar 18) *(calendar event set)*
- [x] Move-out inspection — Mar 19, 3:00–3:30 PM *(scheduled)*
- [ ] Return 2 keys at inspection (1 fob + mail key) — confirm plan with Urby
- [ ] Cancel Lemonade renter's insurance — Mar 20 *(calendar event set)*
- [ ] Pick up Hertz rental car — Mar 20, 9:00 AM, 543 Cary Ave, Staten Island
- [ ] Pack rental car — Mar 20, 1:00–4:00 PM (elevator slot)
- [ ] Dispose of / donate large items: couch, mattress, leg rest, bed frame (coordinate with Urby)
- [ ] Final walkthrough of apartment
- [ ] Drive to Boston — morning of Mar 21
- [ ] Return last key fob (confirm drop-off method with Urby)

### Post-Move First Week (Mar 22–28) — Settling In
- [ ] Drop off Hertz at Boston Logan — Mar 22, 11:00 AM *(calendar event set)*
- [ ] Unpack and set up computers/work station
- [ ] Confirm work setup is functional (monitors, wifi, desk)
- [ ] Look into upgrading Boston apartment wifi

### Within 30 Days of Move (by Apr 21) — Admin
- [ ] Update address: bank/financial accounts (Chase, credit cards)
- [ ] Update address: Anthropic, Gemini/Google, Perplexity
- [ ] Double-check Rippling for any other fields to update
- [ ] Request medical records from Medvidi
- [ ] Find new family medicine PCP in Boston (can manage ADHD prescription)
- [ ] Cancel dentist appointment (June 19) and request records transfer
- [ ] Review old Emblem Health coverage for anything needing transfer
- [ ] Set up renter's insurance for Boston apartment
- [ ] Find a gym in Charlestown/Boston area
- [ ] Find a barber nearby
- [ ] Ask parents to send SS card and birth certificate to Boston address
- [ ] Look into health insurance coverage in MA (confirm network works)
- [ ] Shop for new desk with girlfriend

---

## Urby Apartment Requirements
- [x] USPS mail forwarding set up
- [ ] Submit forwarding address to Urby
- [ ] Fill out and email Notice to Vacate PDF
- [ ] Link Moved.com on Urby Resident Portal
- [x] Moved app tasks — all 6/6 complete
- [x] Move-out inspection scheduled (Mar 19, 3:00–3:30 PM)
- [x] Elevator/loading reserved (Mar 20, 1:00–4:00 PM)
- [x] Resident Portal balance paid
- [x] Auto Pay cancelled
- [ ] Cancel Verizon wifi
- [ ] Cancel Lemonade renter's insurance (Mar 20)
- [x] No wall holes to patch
- [x] Bathroom cleaned
- [ ] Finish cleaning: oven, microwave, fridge, general wipe-down
- [ ] Return keys: 2 fobs + 1 mail key ($50 each if not returned)
- [ ] Coordinate large item disposal (couch, mattress, leg rest, bed frame)
- [x] No bicycle in storage
- [x] No storage unit
- [x] No Littleman Parking

## Questions for Urby (One Call)
- [ ] Lease ends Mar 25 but moving out Mar 20–21 — any issues or prorated credits?
- [ ] Donation options for unwanted items (couch, mattress, leg rest, bed frame), or just refuse room?
- [ ] How to coordinate large item disposal/bulk pickup timing?
- [ ] Can I return 1 fob + mail key at inspection (Mar 19) and keep 1 fob until Mar 21? Where to drop off the last fob?
- [ ] Confirm no additional charges between move-out day and lease end

## Moving Logistics
- [x] Hertz rental car booked: 7-passenger SUV (Nissan Pathfinder), confirmation L5151252687
- [x] Pickup: Mar 20, 9 AM — 543 Cary Ave, Staten Island (Mon–Fri only, closed weekends)
- [x] Drop-off: Mar 22, 11 AM — Boston Logan Airport, 156 Tomahawk Drive (24/7)
- [x] Elevator slot reserved: Mar 20, 1–4 PM
- [x] COI provided to building
- [ ] Confirm Hertz reservation 2 days before (by Mar 18)
- [ ] Pack blankets/towels around TV and electronics for transport
- [x] No storage needed

**Items to transport:** computers (Mac Mini, MacBook Pro, Alienware), monitors, TV, computer chair, clothes/shoes, plastic drawer tower, kitchen supplies, desk (disassembled as backup)

**Items to donate/toss:** couch, mattress, leg rest, bed frame

## Address & Account Changes
- [x] USPS mail forwarding
- [x] Amazon address updated
- [x] Subscriptions updated (Graymatter, Butcher Box)
- [x] Tax withholdings updated (Rippling)
- [x] Work address updated (Rippling)
- [ ] Bank/financial accounts (Chase, credit cards)
- [ ] AI accounts: Anthropic, Gemini/Google, Perplexity
- [ ] Double-check Rippling for any remaining fields
- [x] Driver's license — keeping NY (valid until 2029)

## Health & Medical
- [x] Prescription: 90 tablets 20mg IR sent to Rosebank pharmacy (30-day supply at 3x/day)
- [ ] Pick up prescription from Rosebank pharmacy before leaving
- [ ] Request medical records from Medvidi
- [ ] Find family medicine PCP in Boston (Dr. Cat recommends family med over internal med for ADHD rx continuity)
- [ ] Cancel dentist appointment (June 19) and request records transfer
- [ ] Review Emblem Health coverage for anything needing transfer
- [ ] Look into health insurance coverage/network in MA
- Dr. Cat's contact info available if needed during transition

## Utilities & Services
- [ ] Cancel Verizon wifi (target: this week)
- [ ] Cancel Lemonade renter's insurance (Mar 20)
- [x] Boston apartment: internet, electric, gas, water — all set up
- [ ] Set up renter's insurance for Boston apartment
- [ ] Consider upgrading Boston wifi

## Work (The Block)
- [x] The Block notified of move
- [x] Tax withholdings updated
- [ ] Notify manager about Mar 20 (available but no meetings 1–4 PM)
- [ ] Double-check Rippling for remaining updates
- [ ] New desk — shopping with girlfriend (current desk as backup)
- [x] Work equipment transporting self

## Tech & Backups
- [x] All 3 machines backed up to Google Cloud (5TB)
- [x] Packing materials ready (plastic tubs, blankets, original monitor boxes)

**Devices to transport:** Mac Mini, MacBook Pro, Alienware desktop, monitors, TV

## Personal & Life Admin
- [ ] Find a gym in Charlestown/Boston
- [ ] Find a barber nearby
- [ ] Ask parents to send SS card and birth certificate to Boston
- [x] Passport — accessible, bringing to Boston

## Budget
- Estimated total: ~$450–500 (rental car, gas, tolls)
- Packing/cleaning supplies: already owned

---

## Notes
- Lease officially ends March 25 (Wednesday) but physically moving March 20–21 (weekend) to avoid taking off work
- Girlfriend already in Boston at 115 Elm St, Apt 1, Charlestown, MA 02129 — apartment is move-in ready
- Dr. Cat (psychiatrist) leaving Medvidi — gave personal number, offered to help during transition. License is NY-only, cannot prescribe in MA
- Medvidi can still provide care in MA with a different provider if needed as a bridge
- Hertz SI pickup location is closed weekends — no fallback if Friday pickup has issues, so confirm reservation early
- Urby building: Bldg 7-2062, 7 Navy Pier Court, Staten Island, NY 10304
- Hertz confirmation: L5151252687
