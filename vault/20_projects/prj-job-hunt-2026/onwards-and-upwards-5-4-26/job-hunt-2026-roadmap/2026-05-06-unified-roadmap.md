...
> **Status:** Step 0 (error analysis) ✅ complete 2026-05-10. Substack drafts ✅ complete (Sedaris primary + Kerouac variant). Build plan ✅ at [`2026-05-10-eval-suite-build-plan.md`](2026-05-10-eval-suite-build-plan.md). **Workstream A code shipped ✅ 2026-05-12** at [`evals/vault-synthesizer/`](../../../../evals/vault-synthesizer/) — branch `eval-suite-2026-05-12`. **Workstream B synthesizer patches shipped ✅ 2026-05-12** — suite at 7/10 post-fix (up from 1/10 pre-fix baseline). A13 (Loom) and A14 (Substack publish 2026-05-22) remain in Sean's hands. B7 (5-night live-synth gate) running in parallel. Workstream C (Substack-Drafter agent) begins ahead of original "post-employment" schedule per 2026-05-12 decision (see Task 9 update below).
...
- Workstream A: 14 commits (`dc90f7e` plan ingest → `783bc31` CHANGELOG/CLAUDE/README). Files at `evals/vault-synthesizer/`: cases.yaml (10 cases), deferred-cases.yaml (11 cases), runner.py (~210 lines), conftest.py, failure-modes.md, EXPLANATION.md, README.md, references.md, last-run.md (auto-generated), traces/{phase-0-reads.md, phase-0.5-spike.md, baseline-run-2026-05-12.md, coded-traces.md, fixtures/}.
- Workstream B: 8 additional commits (`58c23ff` model_used → `2b9f59c` skip_reason refresh). Synthesizer patches at `agents-sdk/agents/vault_synthesizer.py` (`STATUS_*` constants, `MODEL_USED_VALUES` frozenset, `_normalize_model_name`, files_attempted/succeeded counters, end-of-loop status promotion). Pushover boot check at `agents-sdk/lib/pushover.py` (`PushoverConfigurationError` + `ensure_credentials_or_raise`). Daily-driver helper at `agents-sdk/agents/daily_driver.py` (`render_vault_health` standalone, wired into `build_preamble`).
...
- Branch: `eval-suite-2026-05-12` (continues from Workstream B's 22 commits)
- Sequencing: C0 (preconditions) → C1 (config table) → C2-C7 (TDD modules) → C8 (launchd plist) → C9 (supervised pilot drafts) → C10 (CHANGELOG/CLAUDE/README updates)
- Hard precondition for **enabling** the agent (running C9 pilots): B7's 5-night gate. C0-C8 can be built and tested offline without B7 closing because the agent ships default-disabled and TDD tests use mocked synthesizer output.
- Sean reviews every pilot draft before installing the launchd plist (C9 step 5 is gated on his approval).
