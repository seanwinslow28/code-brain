...
| Tier 2 — Cluster-and-sample retrieval | **shipped-bug-fixed-awaiting-2026-05-18-nightly** | 2026-05-16 | 2026-05-16 (initial); 2026-05-17 23:xx (routing-bug fix) | New `lib/retrieval_diversity.py` (cluster_and_sample + build_embedding_query), 14 new tests (all pass), full pytest suite 550 passed with no Tier-2-related regressions. `vault_indexer.search()` gained `include_embeddings` kwarg (backward compatible); `run_synthesis` retrieves pool of 50 → clusters with HDBSCAN → samples 2 per cluster (≤15 total); `SynthesisResult.clusters_sampled` surfaces in manifest. `hdbscan>=0.8.40,<0.9` added to pyproject. **2026-05-17 verification surfaced a routing bug** — `_default_retriever_factory._retrieve` shim only accepted `(query, top_k)`, so the synthesizer's `retriever(query, k, include_embeddings=True)` call raised `TypeError`, the silent fallback at line 603-609 fired, and the pool came back embedding-less → line-617 isinstance guard False → legacy top-K. Fixed by extending the factory shim to forward `include_embeddings` (default `False`); two regression tests added (`test_default_retriever_factory_accepts_include_embeddings_kwarg` pinning signature + `test_default_retriever_factory_forwards_include_embeddings` pinning behavior with a mocked `search`). Neighborhood suite 78 passed. First real signal now: **2026-05-18 02:30 AM nightly run**; verification gate is `clusters_sampled ≥ 3`. |
