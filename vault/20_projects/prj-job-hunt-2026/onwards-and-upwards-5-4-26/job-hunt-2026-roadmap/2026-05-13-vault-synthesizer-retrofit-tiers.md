...
| Tier 2 — Cluster-and-sample retrieval | **live-and-passing-gate (1/3 healthy nights)** | 2026-05-16 | 2026-05-16 (initial); 2026-05-17 (routing-bug fix); **2026-05-18 first healthy nightly** | Tier 2 retrofit shipped 5/16 with `lib/retrieval_diversity.py` + 14 new tests + `hdbscan>=0.8.40,<0.9`. Routing bug surfaced on 5/17 (`_default_retriever_factory._retrieve` missing `include_embeddings` kwarg) was fixed same-day with two pinning tests. **2026-05-18 nightly is the first run with the new path actually exercised: `clusters_sampled: 38` (gate is ≥ 3, easily clears). Duration `2804.9s` is +85s vs the pre-fix 2715s baseline — HDBSCAN overhead visible in telemetry, corroborating the manifest field.** Spot-check of `agentic-engineering-signal-in-product-development.md` shows real 3-cluster cross-domain synthesis (Autonomous Agent Fleets × Intent Engineering × AdOps Automation) with verbatim quotes from three distinct source files. Tier-3 trigger gate ("≥3 consecutive healthy Tier-2 nights"): 5/18 ✓, 5/19 pending, 5/20 pending → earliest plausible Tier-3 evaluation morning of **2026-05-21**. |
