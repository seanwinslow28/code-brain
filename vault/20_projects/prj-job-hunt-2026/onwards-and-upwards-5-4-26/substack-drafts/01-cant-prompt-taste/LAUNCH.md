...
- **Copy:** ready. Voice chain passed (layoff suppressed per Do-Not-Promote; gauntlet cheese line swapped; analyzer CV 0.757 / MATTR 0.82, in your baseline; zero em dashes).
...
## Step 1 — Generate the hero image (on your Mac)

The sandbox is firewalled off the Gemini API, so run this locally. Prompt is saved at `images/hero-prompt.txt`.

```bash
cd ~/Code-Brain/code-brain
python3 .claude/skills/gemini-image-gen/scripts/generate_image.py \
  "$(cat 'vault/20_projects/prj-job-hunt-2026/onwards-and-upwards-5-4-26/substack-drafts/01-cant-prompt-taste/images/hero-prompt.txt')" \
  --output "vault/20_projects/prj-job-hunt-2026/onwards-and-upwards-5-4-26/substack-drafts/01-cant-prompt-taste/images/hero.png" \
  --aspect-ratio 3:2 \
  --env-file .env
```

It's a grotesque LinkedIn-guru with the caption "Your setbacks aren't setbacks. They're setups." (cheese-caption + grotesque satire, per the cheese-bank satire-redemption rule). If the first render misses, describe a tweak and re-run (NB2 edits conversationally) — don't start from scratch.
