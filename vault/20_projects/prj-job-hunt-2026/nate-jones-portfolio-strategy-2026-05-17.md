...
> **Thinking:** Ranked by impact-per-week. Top of list = ship-in-14-days asymmetric wins (extract work that already exists). Middle = 2–4 week net-new artifacts that close specific gaps. Bottom = 2–3 month flagship. Hard constraints: every project lives outside `code-brain/`; every project maps to ≥2 of Nate's seven and ≥1 of the 2027 premium capabilities; every project includes real eval + cost + documented failure; ≥1 project addresses an Enterprise PM concern Nate doesn't cover (vendor eval / build-vs-buy / governance / change-management); ≥1 project is a public-facing artifact (Substack/talk/README), not just code.
