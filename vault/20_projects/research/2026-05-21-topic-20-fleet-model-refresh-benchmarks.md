...
**ADOPT — `gemma4_26b-32k` @ Alienware as the Tier C batch/async model.** 7-day Pattern E soak ran 2026-05-26 → 2026-06-03 (9 days, 17 datapoints, summarization workload). Result: **17/17 clean** — 0 thinking-token leakage, 0 truncation/parse-fail, 3-paragraph structure held every run, median quality "thinking-partner shape" on every sampling day, and no throughput drift over the window. The only soft miss was mean throughput **29.6 tok/s** vs the 30 tok/s adopt threshold (and two same-day >2× wall ratios) — both are workload-bound, not instability: `corr(num_ctx, tok/s) = −0.75`, so the slow runs are simply the large-article runs, and the soak corpus skews larger than the benchmark prompts that produced the 39.8 tok/s headline. Adequate for an async batch tier; **not** recommended for interactive use. Routing/`task_map` wiring deferred to a follow-up session per the soak protocol. Full gate-table + analysis: [[2026-06-03-topic-20-tier-c-soak-closeout]].
