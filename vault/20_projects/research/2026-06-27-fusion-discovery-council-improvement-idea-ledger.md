...
| E6 | Auto web-supplement (BACKFILL Stage 5) — ✅ **SHIPPED + prod-validated 2026-06-28** (dogfound + fixed a crash; keep default-on) | 4 | 2 | ★★ | Evidence/Output | **DONE** |
...
- **E1** — entailment gate v2 (atomic-claim + NLI; substring as pre-filter; precision/recall on the scorecard). **When it lands, also fix E6's self-referential gate call** in `backfill.py` (`quote_supported_at_url(cited_quote=r.quote, fetched_text=r.quote)` → always True) to pass the *gap/blind-spot as the claim* + the fetched page text, so BACKFILL actually inherits relevance-vetting (today it's an inert no-op — safe by construction, but E1 won't flow to it for free).
