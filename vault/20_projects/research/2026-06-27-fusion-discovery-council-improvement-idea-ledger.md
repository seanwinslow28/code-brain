...
| E6 | Auto web-supplement (BACKFILL Stage 5) — ✅ **SHIPPED → agent-layer pivot 2026-06-28** (default now agent WebSearch/WebFetch $0; in-CLI Exa/Brave opt-in) | 4 | 2 | ★★ | Evidence/Output | **DONE** |
...
- **E1** — entailment gate v2 (atomic-claim + NLI; substring as pre-filter; precision/recall on the scorecard). **Now CORE-VERIFY only:** the 2026-06-28 agent-layer pivot moved BACKFILL relevance-vetting to the agent (+ the `verify_supplement` verbatim backstop), so the old self-referential Stage-5 gate cleanup is closed — E1 no longer needs to flow to backfill. **Cost-model is an open DECISION-with-Sean** (local NLI vs OpenRouter judge-per-claim vs subscription agent — do NOT default to paid per-claim).
...
- **E6** — ✅ **SHIPPED 2026-06-28 (b127aed, PR #97)** — auto web-supplement / BACKFILL ([prompt](../../../docs/prompts/2026-06-27-fusion-discovery-council-web-supplement-stage-prompt.md)). *Was a pull-forward candidate; built first per the audit.* Original rationale: it's spec'd and additive, so it can land as early as Phase 1 to feed D1's *why-now / proposed-bet* slot with solution-side evidence. **No hard dependency on E1** (safe by construction — deterministic extraction can't fabricate), but it must call the **shared verification helper E1 will own**, so its relevance vetting upgrades for free when E1 lands. Default-ON → watch the *verified-but-ignored* counter-metric so the supplement doesn't become an unread appendix. **Dogfood (2026-06-28, 4 live P&P runs):** keep default-on — fires reliably, zero leakage into the ranked angles, and was the *only* evidence source when GATHER came back empty (T4). Found + fixed a Stage-5 crash (Brave's 50-word query ceiling → 422; now clamped + per-gap try/except). **Two follow-ups opened:** (1) ✅ **RESOLVED (#99)** — the post-fuse $0-spend crash (~$1.9 under-counted 2026-06-28) now threads billed cost into a typed failure (mirrors `FusionError.cost`); only the manual OpenRouter ledger reconcile remains. (2) **E6 query-quality** — prefer gap-keywords over the full topic + skip pure-meta gaps (the relevance-drift the LEADS label flags; now handled natively by the agent backfill). **Architecture pivot (2026-06-28, ✅ Task 1 DONE):** backfill moved to the **agent layer** — Claude Code's own WebSearch/WebFetch on the Anthropic subscription ($0 marginal), run after the council, replacing the in-CLI Exa/Brave deterministic version (which goes default-off/opt-in). Agent-driven is cheaper (subscription, not Exa/Brave) AND higher-quality (intelligent synthesis vs keyword extraction) AND it vets relevance natively — which **moots the E1-coupled-backfill cleanup** and **de-risks the cost-recording hole** (backfill was the only post-fuse network call). Plan: continuation prompt `docs/prompts/2026-06-28-fusion-discovery-council-continuation.md`.
