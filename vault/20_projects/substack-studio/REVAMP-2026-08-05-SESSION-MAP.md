...
> **◐ MOSTLY DONE 2026-08-08.** (A) PAGES: both chain drafts written and committed (`pages/start-here.md`, `pages/about.md`, status `chain-draft-awaiting-sean-rewrite`; old drafts date-suffixed into `pages/old/`, nothing deleted; shape locks in `docs/superpowers/specs/2026-08-08-substack-pages-relaunch-design.md`). AWAITING Sean's hand-rewrite → fold into voice-samples → publish as Substack pages. (B) PROFILE: 3 old posts unpublished (drafts, not deleted); ALL 9 restacks removed (Sean widened scope: clean slate, not curation); bio replaced ("I run experiments on AI so you don't have to trust anybody's magic prompt. Verdicts published, faceplants included."); theme switched Profile → Custom; Introduction set (subtitle line + verdict promise, 2-3 sentences). **Platform finding:** @seanpwins is Substack's newer profile-publication type — NO publication-name/subtitle text fields exist (checked Settings, website editor, Edit Profile); profile Name + Bio double as site name + subtitle, and "Pencil & Prompt" can only appear via a header Wordmark image (S2 masthead asset) or a profile rename. Sean deferred the wordmark ("leave as Sean Winslow for now"). REMAINING: publish the two pages after hand-rewrite; build nav (Home · Start Here · Building the Ladder · About · GitHub · Portfolio — needs pages/sections to exist first); apply S2 assets (wordmark/avatar/heroes) when Sean opts in.
