...
### Trace kit 0.7.0 — verdict buttons carry color and size, and a Defer key (2026-09-25)

Sean's ruling on the two points where Husain and Shankar's own review-interface spec cut against the kit's ratified ink-only verdicts, surfaced by the legible-evals research ([#305](https://github.com/seanwinslow28/code-brain/issues/305)). In the shared viewer (`productcraft/trace/tracekit/viewer.py`, inherited by the content machine's kit): the **pass** button takes the portfolio's drafting ink (`#2F5D7C`, lifted ink `#6BA3C9` at night) and the **fail** button a red pencil (`#9E3B2E`, `#D9857A` at night), both set larger than every other button, and each row's verdict word carries the same color; the train, the matrix and the track stay ink. A third button, **defer** (`d`), is a recorded non-verdict: `labels.md` accepts `defer`, but a deferred row is never counted as labeled, still reads as waiting, never reveals a blind pair, and has its own filter. Record: `productcraft/trace/DESIGN.md` §15. Tests: 183 + 24 passing; the committed synthetic sample is re-rendered.

### Content machine trace kit — one record per stage invocation, on the shared kit at 0.6.0 (2026-09-22)

Resolves the Content Machine build map's trace ticket ([#291](https://github.com/seanwinslow28/code-brain/issues/291)), the build of the design [#261](https://github.com/seanwinslow28/code-brain/issues/261) resolved jointly with Productcraft's [#272](https://github.com/seanwinslow28/code-brain/issues/272). **The shared kit becomes studio-agnostic rather than being forked**: `productcraft/trace/tracekit/studio.py` adds a `Studio` profile holding the one set of things two studios disagree on — numbered stages, kinds, the kinds that own `## Moves`, gate seats, repo path prefixes, the item-id shape in a Moves line, the taxonomy file, the header file and one structure check (rung-0 line 8, registered by key) — and the loader, checker and viewer read it from the engagement. `PRODUCTCRAFT` is the default, so nothing that called the kit changes meaning (172 → 178 tests, the synthetic engagement re-rendered with the studio named in its footer). **The content machine's copy** at `.claude/skills/content-machine/trace/` is then only what the machine writes for itself: `machine.py` (stages 0 Oracle → 6 Lessons in the machine's own numbering, ten kinds `sweep` · `pick` · `topic` · `stimulus` · `interview` · `shape` · `gate` · `rewrite` · `proofread` · `lesson`, the seats, rep ids like `Rep 7e` matched as whole tokens so `Rep 7` never covers `Rep 7e`), `check.py` and `render.py` importing `tracekit` from `productcraft/trace/`, the record and labels templates, an honestly empty `taxonomy.md`, and a synthetic three-card X deck (`tests/deck_synth.py`, invented handles, refuses to write under the private brain) with 24 tests. Records live in the git-ignored `pieces/<run>/trace/`, so a run's trace is private by construction. **Line 8 is the machine's own and it is the constitution made checkable**: per shape, no banned file among its hashed inputs, a transcript or block among them, every slot actually opened per `## Corpus read`, at least one gate fired on the card at the hash the shaper returned and transcribed onto its `checks`, and a stage-5 pick or rewrite that took the card (a note while `run.md` says `closed: null`). **The shaper's `## Moves` is elicited in a follow-up turn after the draft is on disk, never in the drafting prompt** — so the clean context stays byte-identical and #258's falsifier and #259's arming gate measure what they measured before. SKILL.md gains *The trace*, a close-the-trace step, the Moves rule under *The shaping context* and a private-brain row; the X contract's deck close gains step 9; the machine README's table gains the `pieces/<run>/trace/` row. Runtime-impact ruling: no retirement; registry scan clean.

### Productcraft runtime registry and trial protocol — kit 0.5.0 (2026-09-21)

Resolves the Productcraft build map's registry ticket ([#286](https://github.com/seanwinslow28/code-brain/issues/286)). `productcraft/templates/runtime-registry.md` is the table every -craft team shares: one row per harness × provider route (Claude Code by Agent tool and headless, Codex CLI on OpenAI, to OpenRouter and to Ollama, Pi, Hermes, Gemini CLI), each carrying its verbatim launch form, sandbox and disk reach, a source-confirmed `meter_source`, its steps on the #267 ladder, its **standing** and what is installed on this machine today. Standing is a four-step ladder a runtime never skips — *ruled* (placed by #267), *unmeasured* (registered, no labeled pass on real work), *substitution-eligible* (one blind-labeled, rung-0-clean trial) and *baseline-eligible* (rung 1 applied plus a count floor the trials ticket sets) — so an unmeasured runtime can serve a dated owner-approved substitution or a shadow trial and never becomes a seat baseline by drift. A trial runtime has no tier. The six-step trial protocol (register, declare at Open outside the funded cap, fire on identical hashes, label blind, promote by a `promoted:` line in the trial record's notes, count at Close) runs on real work only, per #287's constraint. Ruled out with reopening conditions: Aider (no read tool), Jev as a seat runtime (non-generative), the Pi subscription-auth bridges, Codex `ultra`, `gpt-6-astra` as a default (rejected by the installed 0.146.0), and every memory add-on. Five rulings are adopted pending Sean's ratification and ticketed.

Kit **0.5.0**: `trace/registry.py` derives the registry's § Numbers — per runtime and per runtime × seat with its family, labeled passes as counts, the rung-0 clean count, medians over measured passes, trials, promotable and promoted — so no number in that public file is typed by hand; the first run reads pc-eng-001 (Opus 18 of 22 labeled pass, 19 of 22 rung-0 clean; Codex 3 of 3). The meter-source vocabulary grows from three values to eight, one per row, and a test fails if the registry and `METER_SOURCES` drift. The master skill's Route names each runtime's row and standing, its dispatch forms are the registry rows verbatim (Codex canonical form gains `--json` for the split pair, the footer stays accepted, `--model` is always explicit), "one tier up" is defined on ruled rows only, the cost line gains an API-billed row form, and Close gains **Trace 4 — registry numbers**. Glossary: Runtime registry, Runtime row, Standing, Registry numbers, Meter source, Promotion. 172 tests.

### Productcraft trace kit 0.4.0 — rung 1 opens: the first seat failure modes, read from pc-eng-001's labels (2026-09-21)

Resolves the Productcraft build map's rung-1 ticket ([#299](https://github.com/seanwinslow28/code-brain/issues/299)). The taxonomy's § Seat failure modes was honestly empty until someone had read about thirty traces; pc-eng-001's 33 labels landed on #295, so the reading happened. **Two modes are coded**, with their provenance carried on every count — the 33 verdicts were coordinator-drafted and Sean-approved, and the names were proposed by that coordinator for Sean to ratify. **`unobservable-measure`** (a measure, kill condition, trigger or safeguard defined on an event the pilot's own rules cannot produce or deliver) leads three of the five fails and appears as eight distinct defects across four drafting seats — an outcome left open to a prompt channel, a classifier resting on a reconciliation no contact rule authorised, a harm pause with no route to a phone before day 14, a pre-registered metric on a record field that does not exist. **`overclaimed-pointer`** (a citation or status claim that says more than its source holds) leads one fail and appears as eight distinct defects across two seats — five evidence rows bounced at co-sign, a pointer into an overwritten revision, two memos claiming a ratification their ledger entries read `null` on. Two more shapes recur and stay **sighted, not coded**, because no fail is led by them: an *invented threshold* (two defects, one seat) and *contradicts a standing rule* (six defects, three seats); a shape earns a code when it leads a fail in the second engagement. Counts are given two ways — coded fails, which rung 0 checks, and distinct defects, which say whether a mode recurs — and a fail whose lead shape is sighted-only (pass-25) keeps a blank code rather than being stretched onto a mode. Thirty-three labels is the floor for starting a taxonomy, not for trusting one, and the file says so.

Two checks ride along. **#296's process-waste prediction** (1 `kit-induced`, 1 `stale-restatement`, 1 `overstated-scope`, 0 `manufactured`) holds as findings named in the gate-pass critiques and reads 0 / 0 / 0 / 0 as coded rows, which is what the taxonomy's own rule requires. **#296 clause 2's "4 of 4 residuals of the revision's own repairs"** was re-read against the re-audit's verification table by a session other than the one that measured it: M8 sits in M4's repair row, N8 in M3's, N10 in the amended adoption of N6, and N9 in M5/M6's — three clean, one soft (N9's rewritten cell was repair-touched but its root, the unscanned candidate list and the no-competitor claim dropped from C4, predates the repair).

Kit **0.4.0**: `tracekit/taxonomy.py` reads **every** code table in `taxonomy.md`, not the first, so the seat family sits under its own heading beside the process-waste family; the viewer's *Failure taxonomy* growth slot (DESIGN.md §10) now **fills from the labels file** — one line per mode with its count and the rows behind it, a row's `failure_code` linking to its mode, a code outside the taxonomy listed apart as free text — and stays in its empty state until a row carries a code. The labels template, kit README, studio CLAUDE.md and glossary (seat failure mode, sighted shape, and the two modes) updated. **160 tests** (up from 156).

### Productcraft trace kit 0.2.0 — the gaps the first train exposed (2026-09-21)

Resolves the Productcraft build map's kit-and-template ticket ([#297](https://github.com/seanwinslow28/code-brain/issues/297)), which blocks the second engagement. The first train ran on kit 0.1.0 and the check reported things that had not happened. **Two of its findings were the kit's fault, not the record's.** A repair redrafts an artifact in place (#274), so the file it hands back still carries every citation its earlier revisions made — and the corpus line charged the *repairing* pass with reading all of them. A citation is now accounted for by this pass's `## Corpus read` **or** by that of an earlier pass which wrote the same artifact; reads travel along one artifact's revision chain and never sideways. And a seat's inputs include the studio's own files — a seat contract, a lane manifest, a template — which live in the repo, outside the engagement, and keep improving after a train closes; pinning their hash meant the ticket that *fixes* a template made a closed engagement read as tampered with. A repo-path input no pass in the engagement wrote is now provenance rather than a chain link: when it has moved, it is counted **unverifiable** and named, never rewritten to match, exactly as an overwritten revision's Moves are. Everything the engagement itself wrote stays strict, including a repo path some pass recorded as an output. On the real pc-eng-001 both lines now PASS — 53 inputs across 11 files named as moved machinery, three of which had already drifted before this ticket touched anything.

Four more gaps closed. **Record kinds** gain `open` and `readout` (the first train reused `close` for both): `open` is what anchors the chain — the brief, the frozen evidence and any anchoring entries enter the hash history as its outputs — and `readout` is the post-gate human pass, a real invocation outside the pass budget. Both join `close` as coordinator kinds with no drafting conversation to withhold. **Meters** may now be a single `total`, which is what the Agent tool's usage field and the Codex footer each actually report; the split pair stays available and the viewer prints a total as *as the runtime reported it, not split*. Neither form permits a guess. **Ledger entries ride the chain**: an entry a pass wrote is a `- path:` block with its hash beside its `- id:`, and a coordinator's supersession mark is an output of the superseding pass. **`nextid.py`** hands out the next `dNN`, counting an id as claimed by a file on disk *or* a record that reserved it, and reporting the gaps (left alone, never reused) separately from ids reserved but never written — the reserved blocks of the first train are why two passes can otherwise claim one id.

`## Moves` is now **declared machine-read** in all eight seat templates, after a seat filled the section with prose the checker could only report as malformed; the Moves parser learned to skip whole `<!-- … -->` blocks so the declaration itself parses. `strategy-pov.md` gains the two sections the Strategist added by hand and the template lacked — **`## Diagnosis`** (Rumelt's kernel: one binding constraint and a guiding policy, before any statement of what to do) and **`## Framing only`** (the craft questions this bench does not answer, written even when empty) — each with its own red-team attack. The decision memo's `state:` is now the ledger entry's `status:` verbatim (`proposed | decided | superseded | reopened`) rather than a second vocabulary. The master skill's Gate and Close steps carry the readout lines, and the templates README indexes `readout.md` and `readout-glossary.md`. **143 tests** (up from 121).

### content-oracle — a second gate: was anything actually learned? (2026-09-13)

The animation card scored 16 of 18 and banked two weeks running. Week 2's stated reason — re-deck it when the project is active again — was tested and falsified: the project *was* active, eight beats locked in one session, and he banked it anyway. His real reason names two filters the skill did not have. First, **the slip is not the finding**: he already knew that a back-facing start frame makes the model invent a face, and the incident was only that he didn't notice which frame was in use. *"It was something I already knew... it was just a slip up on my end."* **In the record those two things are indistinguishable** — a rule already held, an output that breaks it, and a fix leave exactly the same commit, daily-note lesson and working rule as a rule discovered by breaking something, so no improvement to the sweep can separate them and the check has to be a gate. Second, **specific is not new to anybody**: *"pretty well known at this point."* The card scored Depth 3 and Framework 3, and neither signal asks whether a practitioner would raise an eyebrow — Depth measures specificity, Framework measures transfer, and a well-known rule is both and still worth nothing. [`.claude/skills/content-oracle/SKILL.md`](.claude/skills/content-oracle/SKILL.md) gains **Was anything actually learned?** beside `Interviewability`, as a gate rather than a seventh signal (#162's lesson is that a new tier is the wrong fix): of every done-thing card, was the rule already in his head before the incident, and would a practitioner already know it. It **bans restatement and never evidence** — putting a number on a known rule passes, and so does falsifying one, which is what the contested-claim query shape exists for. It runs at step 5 ahead of the cold read. The gate is recorded as explaining one of week 3's five banks and is explicitly not stretched to the rest.

### Productcraft trace kit — pass records, a labels file, the rung-0 checker and the eval viewer (2026-09-13)

Resolves the Productcraft build map's "Build the trace kit" ticket ([#290](https://github.com/seanwinslow28/code-brain/issues/290)), the four day-one pieces of the evals-and-trace design ratified on [#272](https://github.com/seanwinslow28/code-brain/issues/272), with the viewer built to the DESIGN.md approved on [#292](https://github.com/seanwinslow28/code-brain/issues/292). New in `productcraft/trace/`: **`record-template.md`** (one immutable markdown file per invocation with the locked frontmatter — pass, seat, kind, fixed stage, runtime plus verbatim launch form, instants, meter plus source, every input and output with its sha256, withheld, raw-log pointer, checks, `triggered_by`, `shadow_of` — and the Corpus read / Moves / Notes sections; who writes which field); **`labels-template.md`** (one file per engagement, rows keyed by pass id: pass | fail, first failing stage, critique, blank failure code); **`check.py`**, the rung-0 checker (nine deterministic lines, no model, no network, stdlib only, exit 1 on any FAIL with each finding naming the pass and the thing: records parse, every pass has a record, every pass has a label row, input hashes match disk *or a recorded prior revision with the disk holding the latest one* — the overwrite-in-place rule of #274 made that the necessary form, and a file edited outside a pass now fails — cited corpus files were opened, every move names an existing upstream item with splits new and merges sourced, meter present or UNMEASURED, each stage has one draft, its fixed auditor's audit and its required co-sign, and trials are blind with hash-identical inputs, verified against the pair's own rows in the rendered page); **`render.py`**, the viewer renderer (`tracekit/viewer.py`: one self-contained offline HTML per engagement into `trace/eval.html`, the ratified prototype's markup, styles and keyboard loop made data-driven — prose reading line composed from the labels, transition-failure matrix derived from first failing stage alone, the train, folded rows with in-page labeling and *Copy label rows*, the three growth slots; a blind pair's runtime, launch form, meter source and log path are absent from the file until both labels land). `tracekit/` holds a stdlib YAML-subset frontmatter parser, the labels and Moves parsers, and the engagement loader that resolves record paths against the engagement folder or the repo. Honest limits are stated rather than hidden: a pass whose artifact was later overwritten reports its Moves as no longer on disk, and a move whose only possible home is an unreadable revision is counted unverifiable, never verified (a transcript replay is a later rung). **97 tests** in `productcraft/trace/tests/` run on a synthetic engagement — `tests/synth.py` *simulates* the invented "Callboard" train so the records' hash chain is real (repairs overwrite in place, two bounce loops, a blind pair, a coordinator close record) and refuses to write under any `ledger/` path; the same folder is committed at `samples/synthetic-engagement/` with its render. The master skill's Close checklist now carries the **three trace lines** with their commands, and its Run and Open steps point at the record template and the brief header the kit reads; every seat artifact template's `## Moves` block shows the five-op grammar the checker parses (defined once in `artifact-header.md`); the glossary gains pass, pass record, labels file, move, shadow pass, rung and first failing stage; Productcraft CLAUDE.md gains the `trace/` row; the private ledger README's layout gains the `trace/` subfolder (a #290 call — twenty-six pass files at the engagement root would bury the decision entries). The content machine's kit ([#291](https://github.com/seanwinslow28/code-brain/issues/291)) imports `tracekit` from here rather than forking it; `craftwork` extracts the shared home later.

### content-oracle — the card is written to Sean, not to the machine (2026-09-13)

Week 3's deck scored and rendered cleanly and he could not read it: *"I'm having trouble understanding what the hell these spikes are even proposing. Filled with a shit ton of jargon and AI speak... It's writing these down as if it's communicating it to another agent."* **A deck he cannot pick from has failed at its only job, whatever it scored**, so this is a defect in the skill rather than in one run's prose. Cause is structural: the card format was designed as a handoff object — a picked card becomes stage 1's TOPIC CARD — so the Oracle wrote the card for the next stage instead of for the person choosing, and three habits followed. It named things by identifier (`#258`, a sha, *the astra drift*) instead of saying what happened; it reached for an abstract noun wherever a concrete thing existed (*the axis*, *the keeper*, *drift vectors*, *the shaping context*); and its experiment cards never said what he would physically do, so "do this, expect that" described a direction he could not picture doing. [`.claude/skills/content-oracle/SKILL.md`](.claude/skills/content-oracle/SKILL.md) gains **Write the card to him, not to the machine** with a rule per habit: identifiers live on `Evidence:` and nowhere else; a term of art is defined the first time it appears in a card, and **a card may not use a word he would have to ask about**; and every experiment card carries a new mandatory `What you'd actually do:` — the real steps in the order he would take them, ending in a rough time cost — **banking if those steps cannot be written**, which is the `Interviewability` bar applied to the doing. A **cold-read gate** now runs at step 5 across the whole deck before the console is built, read as a stranger to the repo who was not in the session. The ideas bank stores that readable card with no denser second copy: if the terse phrasing carried something the plain one lost, the plain one was written badly. Week 3's thirteen cards were rewritten and re-published to the same console; scores and rankings unchanged.

### `keychain.py set --stdin` — the secret never touches the command line (2026-09-13)

Closes the manual ticket filed the same day. `set` only ever accepted the value as `sys.argv[3]`, so every credential in the fleet had to survive a shell parse: installing the headless `claude setup-token` token failed with `zsh: parse error near '\n'` (the `<token>` placeholder in the documented fix line makes `<` an input redirect with nothing after it), and the Keychain had been holding a **34-character truncated value** that returned `401 Invalid bearer token` — the exact root cause of the 2026-06-20 launchd 401, invisible because the entry existed. [`agents-sdk/lib/keychain.py`](agents-sdk/lib/keychain.py) gains `set --stdin <name>`, which reads the value from stdin (`pbpaste | ...`, `claude setup-token | ...`), strips only the trailing newline a pipe adds, **refuses an empty value or one spanning multiple lines** (pasting a terminal transcript instead of a bare token is how the truncation happened), and keeps the secret out of `~/.zsh_history`. Both forms now print the stored length and never the value — `Stored: claude_code_oauth_token (108 chars)` — so a truncated paste is visible at install time instead of months later. `_cli()` takes an optional `argv` so the CLI is testable; 11 new tests in `agents-sdk/tests/test_keychain_cli.py`. The `--stdin` form replaces the `<placeholder>` instructions in [`lib/auth.py`](agents-sdk/lib/auth.py)'s setup docstring, `scripts/gemini_dr.py`'s and `agents/deep_researcher.py`'s missing-credential errors, `agents-sdk/KEYCHAIN-SETUP.md` (new *Setting a credential* section) and CLAUDE.md's Agents SDK auth paragraph. The real token was re-installed and round-trips at 108 characters; proving it authenticates headlessly with no interactive session to fall back on is still open on the launchd-401 ticket.

### Productcraft bench — seven seat files and the bench README (2026-09-13)

Resolves the Productcraft build map's "Author the seven-seat bench" ticket ([#275](https://github.com/seanwinslow28/code-brain/issues/275)). `productcraft/bench/` replaces its placeholder with a README (the seat table with produces / baseline / audits / co-sign touches, the two closed audit cycles, what an invocation carries, the baselines in one line) and seven seat files on the Systemcraft skeleton — `product-strategist.md`, `discovery-lead.md`, `insights-analytics.md`, `growth-distribution.md`, `business-economics.md`, `delivery-execution.md`, `product-leadership.md` — each with a YAML identity (seat number, `model:` naming the actual runtime per #267's 4 Opus / 3 Sonnet split, `produces` with template and artifact id, `lane` by #270's slug, `audits`, `audited_by`, `cosigns` / `cosigned_by` where a touch applies), a mandate, the contract from #266 (receives / produces / hands forward / boundaries), corpus discipline carrying the ratified three-rung degradation ladder (`full | manifest-only | none`) verbatim across all seven, craft rules (why-A-over-B in the seat's own terms, with the canon line as standing behavior pointed at the seat preamble rather than restated; for Insights, **measures, never decides**), the audit duty with its one-sentence stake, and a curated toolbelt drawn from the ratified plugin-routing table under the raw-material rule. Seat-specific structure the contracts demanded: Insights' two-stance dual touch (co-sign at stage 2, artifact at stage 3 with the outcome-to-metric table as the stage-3 check); the Strategist's co-sign duty on Delivery's OKR translation; Delivery's two jobs (roadmap and handoff verdict in the train, the execution breakdown after Systemcraft's return); Leadership's no-sentence-without-a-field hard contracts and the solo-builder rule; Business flagged tool-poor by ruling with its two required sections template-carried; `create-prd` explicitly kept off Delivery's belt as Systemcraft's artifact. Lane links point at `../lanes/<slug>.md`, which the corpus build ticket creates. Productcraft CLAUDE.md status line updated.

### Productcraft artifact templates — seven seat artifacts, seven gap templates, checks and gates (2026-09-13)

Resolves the Productcraft build map's "Artifact templates per seat" ticket ([#274](https://github.com/seanwinslow28/code-brain/issues/274)). Twelve new public templates in `productcraft/templates/`, each a filled example of the shared machine-readable header defined once in `artifact-header.md` (`status: draft | co-signed | final | audited | stale`, hard-coded `auditor` per the two ratified cycles, `cosign` where a touch applies, `grounding`, `thin_lane`, `adjacent_lanes`, `revision` / `stale_from` for the stale cascade; the coordinator writes the train-state fields, the seat never marks itself final) and each ending with its own **red-team checklist** so every attack the gate runs has a named home. The seven seat artifacts in train order — `strategy-pov.md` (positioning authoritative, OKR outcomes in draft, Gate 1 anchor), `discovery-packet.md` (plan + opportunity solution tree + evidence with a `Grade` column reserved for Insights), `metrics-evidence-plan.md` (the outcome-to-metric table as the stage-3 check), `growth-gtm.md` (positioning quoted verbatim, price as a hypothesis until stage 5), `business-case.md` (three-scenario unit economics, arithmetic stated so a gate can redo it, a kill condition), `outcome-roadmap.md` (OKR translation co-signed by the Strategist, the first shipping slice as the handoff trigger, `handoff_verdict`), `leadership-packet.md` (stakeholder map with Sean split by hat and absent classes stated, memo index, operating-model doc with humans and agents as two entity types and an invocation-record column) — carry the **seven gap templates** the plugin inventory found no skill for as required sections (point-of-view statement; activation and retention playbook; growth-experiment design; unit-economics model; business case; operating-model doc) or as their own file (`decision-memo.md`, one decision per memo, fields aligned to the ledger entry, state mirroring `ratified:`). Beyond the train: `execution-breakdown.md` (intake check → overturned assumptions → epics → stories in a flagged design set and an implementation set → first sprint plan → filed issues → the implementation candidate), `check-record.md` (the artifact of every co-sign and trailing audit, with the **four-rung evidence-strength scale** `observed-behavior / reported-behavior / stated-preference / inferred` and what each may carry), and `gate-findings.md` (the red-team gate's file with Productcraft's typed verdicts `STRATEGY | HANDOFF | TRAIN | AUDIT` × `PASS | PASS WITH ACCEPTANCES | FAIL`, implementation holds, the steel-man, and the whole-train attacks). `handoff-brief.md` gains its Gate 2 checklist. Templates README rewritten as the folder's index; Productcraft CLAUDE.md layout line updated.

### Productcraft master skill — the coordinator, with trailing audits and the stale cascade (2026-09-13)

Resolves the Productcraft build map's "Master skill design" ticket ([#273](https://github.com/seanwinslow28/code-brain/issues/273)); five decisions ratified in session, everything else assembled from the map's earlier rulings. New `.claude/skills/productcraft/SKILL.md` on Systemcraft's precedent (the skill knows the process, the seats know the craft, CLAUDE.md knows the law): five engagement types with selection tests and rosters, a **stage table for the full train** naming what drafts, which co-sign it waits on, which audit fires and which gate or crossing follows at each of the seven stages; **trailing audits** (each fires the moment its artifact is final and runs beside the next draft) with the **stale-cascade rule** (a loopback stales every artifact below it, redrafted in order from the pass budget, cascade size reported first); **Gate 1 after stage 3**, once every outcome has a metric and Discovery's evidence audit has landed, so the four seats that build on the strategy build on a signed-off one; **close and track** for the Systemcraft wait (the train closes administratively, the return date is a rule-8 line, the next Open finds the return in `handoff/inbound/` and proposes the execution breakdown); **Productcraft upstream by default** on requests spanning product and AI-system questions, Systemcraft opening directly only when the product decision is already made; the four escalation triggers, ladder, anchor-vendor gate rule, cap-26 pass budget and dispatch forms from #267; the trace duties from #272 as Close-checklist lines; the ledger push from rule 1; and the coordinator's own four entries named. Inherited Systemcraft law (success measure, availability ladder, self-targeted modifier, red-team protocol, close digest) is stated in one line each and linked, marked for `craftwork` extraction rather than copied. New `productcraft/templates/seat-preamble.md` holds the standing behavior every invocation carries — explain why, name the canon, declare grounding, loop back never rewrite, close with `## Moves`, meter yourself — prepended verbatim to every dispatch and restated nowhere. Glossary gains trailing audit, stale cascade, coordinator, seat preamble, return wait. Excluded from export groups.

### The Systemcraft handoff contract — typed brief, frozen copies, five crossing states (2026-09-11)

Resolves the Productcraft build map's "The Systemcraft handoff contract" ticket ([#271](https://github.com/seanwinslow28/code-brain/issues/271)); six decisions ratified in session. New `productcraft/templates/handoff-contract.md` is the law for both directions and every later -craft team inherits it: the Delivery seat crosses a typed brief of references (`handoff-brief.md`) plus **frozen, sha256-hashed copies** of the six design-train artifacts above Leadership (raw evidence, audits, co-signs and ledger entries stay behind); it fires on a full train when the first shipping slice has a Systemcraft-owned layer, or on an audit finding in a Systemcraft lane, and a **no-handoff verdict** is a ledger entry too; the Codex gate fires on the brief, then the **receiving seat runs a fresh-context intake check** and issues one of **five typed crossing states** (`accepted` / `input-required` / `rejected` / `returned` / `returned-partial`) into both studios' `handoff/crossing.md`; the return after Systemcraft's Gate 2 is the mirror — a typed return note (`systemcraft/templates/handoff-return.md`) plus five frozen copies — read by Delivery at the execution breakdown's Open, which writes `informed_by` and flips overturned entries; Gate 3 waits on a `candidate-note.md`. Systemcraft receives it under a **handoff modifier** on its existing engagement types (master skill Open/Close steps, CLAUDE.md rule 7, `originates_from` / `supersedes_external` on its ledger template, `originates_from` on the PRD template). Glossary gains crossing, frozen copy, crossing state, intake check, return note, no-handoff verdict, Systemcraft-owned layer; both private ledger READMEs split `handoff/` into `crossing.md` + `outbound/` + `inbound/`.

### Productcraft decision-ledger schema with the canon line; both studio ledgers get private remotes (2026-09-10)

Resolves the Productcraft build map's "Decision-ledger schema with the canon line" ticket ([#268](https://github.com/seanwinslow28/code-brain/issues/268)). New public template `productcraft/templates/ledger-entry.md` extends Systemcraft's entry by one section and a handful of fields: ids `pc-eng-NNN.dNN` (permanent, no letter suffixes; Systemcraft's bare ids grandfathered), Systemcraft's *real* on-disk layout codified, a seventh section `## From the canon` after `## Why` that is never omitted (title and idea in the seat's words, `canon:` tag for retrieval), a three-rung `grounding: full | manifest-only | none` degradation ladder, `status: proposed | decided | superseded | reopened` plus a dated `ratified:` field, `consulted:`/`informed:` seats, cross-studio references by id only with each ledger writing its own side, `## Audit` → `## Checks`, and the D+14 outcome rule and `publishable:` flag inherited (a handoff crossing is not outside use). Glossary terms added to `productcraft/CONTEXT.md`. Both studio ledgers, previously local-only git repos with no remote, are now backed up to private GitHub repos; Systemcraft's master-skill Close checklist and both studios' CLAUDE.md rule 1 gain the commit-and-push-the-ledger duty for every session that writes to it.
...
Two layers, the same split the origin gate uses. `sweep.py` (stdlib, no model, $0) gathers from six sources — git commit subjects and bodies with bookkeeping subjects dropped, closed GitHub issues, `vault/daily/`, creative-partner session sidecars, fleet manifests, and open manual tickets — and does not judge; the in-session model reads and scores. A harvester that scored would be guessing at stories from metadata.

**The five signals are Lieberman's, transcribed verbatim from the source** (point of view, story potential, emotional intensity, lesson or framework, depth); the sixth, campaign-spine relevance, is the L7 amendment. Two rulings on top. **Every card must point at one artifact** — a sha, a daily, an issue number, a URL — because an Oracle that synthesized plausible spikes from metadata would poison the well one stage upstream of where the origin gate can see: by the time the interview runs, the fake premise is what Sean is being interviewed *about*. And **spine is a veto, not an addend** — a card scoring 0 on spine banks no matter what the other five say. Sean ruled the external `last30days` scan on for every run, so without the veto the deck fills with other people's news on the weeks his own material is quiet. A high-scoring detour is the most dangerous card in the deck, because it is the one he'll pick.

Unused cards bank to `creative-studio/content-machine/ideas-bank.md` in the git-ignored private brain rather than a vault note, because the cards quote his commits, dailies and sidecars and `vault/20_projects/substack-studio/` is a tracked public path — an ideas note there would publish the sweep a card at a time. The gitignore rule was canary-tested before the file existed; `git add` refuses it. `sweep.py --out` likewise refuses any path git does not ignore.

Probation per L7: **on-demand only, no schedule.** Run day Sunday, week 1 = 2026-08-30, graduation reviewed after 2026-10-04. A weekly reminder (`agents-sdk/scripts/oracle_reminder.py` + `com.sean.oracle-reminder.plist`, Sunday 08:00) keeps a probation week from quietly getting skipped. It is deliberately the dumbest thing that works — stdlib, `/usr/bin/python3` so a broken venv cannot take it down, one fixed email, no model, no SDK, no repo reads. A reminder that swept sources would have smuggled the Oracle past its own probation, and become the thing probation was meant to test.
...
- **vault-critic was dead for 38 consecutive nights (2026-07-04 → 2026-08-11).** Every night: `status: error`, `expansions_written: 0`, `codex_failures == codex_calls`. Root cause: `/opt/homebrew/bin/codex` was a broken symlink into an **empty** `Caskroom/codex/0.130.0/` directory (emptied Jul 7 03:30 by a cask upgrade that removed the old binary and never landed the new one). Fixed by `brew reinstall --cask codex` (now 0.147.0, model `gpt-5.6-sol`); verified end-to-end with a manual critique — 1 article, 0 CLI failures, 69.7s, real expansion written. Note: Homebrew's autoremove took `ripgrep` as a side effect; restored (15.2.0).
- **Why nothing noticed, part 1 — `vault_critic.py` returned 0 on total failure.** `main()` ended in an unconditional `return 0` even when `run()` reported `STATUS_ERROR`, so `launchctl list` showed last exit status 0 for 38 straight nights. Now returns 1 on `STATUS_ERROR` (degraded-but-productive `partial` / `success-empty` still exit 0). Covered by 4 new tests in `tests/test_vault_critic.py`, verified to fail against the pre-fix code.
- **Why nothing noticed, part 2 — the meta-agent wasn't looking.** `ACTIVE_AGENTS` in [`agents-sdk/agents/meta_agent.py`](agents-sdk/agents/meta_agent.py) omitted **both** `vault_critic` and `job_feed`; the fleet-health agent was structurally blind to them. Both added with metadata entries. Verified against the real failure signature: the checker now returns `error`, not `healthy`. Fleet report goes 7 agents → 9.
- **flush had never once completed — 885 invocations, zero successes.** The SessionEnd and PreCompact hooks were setting `CLAUDE_INVOKED_BY=flush` **on the very process they spawned**, and that variable is exactly what flush.py's own recursion guard ([`flush.py:314`](agents-sdk/agents/flush.py)) reads to abort. Every run self-cancelled as `recursion-guard messages=0 duration=0ms` — 482 on the Mac Mini (from 2026-04-23) and 403 on the MBP, on two separate `agent-run-history.csv` ledgers that do not sync. Removed the variable from both hook spawn lines; the guard remains fully intact via `flush.py:339-347` (set around the LLM call, restored after) plus the check at the top of each hook. `session-end-auto-stub.sh` uses the same pattern but is unaffected — `auto_stub_people.py` has no such guard.
- **A second flush bug, unmasked by fixing the first.** With flush finally running, the extraction call hit a `timeout=60.0` wall on the Ollama path (first real runs: 60,069ms / 60,072ms) and a bare `except Exception` converted the timeout into `status=ok, sections=0` — indistinguishable from "the session had nothing worth keeping". Measured on the MBP: a 373-message session (22.5K-char prompt) against `qwen3.6_35b-a3b-32k` takes 50.9s *without* the SOUL prepend; with it, over 60. New `LLM_TIMEOUT_S = 300.0` for both runtime paths, and the handler now logs with `exc_info` instead of swallowing silently. Post-fix run: `ok messages=390 tier=complex sections=5` — all five sections extracted, first daily-log entry the agent has ever written.
- **12 job_feed pollers had returned `all 404` for 52 consecutive runs** (lindy 49) — honestly reported in every manifest, unacted-on. Probed all three supported providers with slug variants. **7 recovered as slug fixes:** `hebbia`→`hebbia-ai` (ashby, 22), `fireworks-ai`→`fireworks` (ashby, 58), `sourcegraph`→`sourcegraph91` (greenhouse, 8), `luma-ai`→`lumaai` (ashby, 50), `captions`→`mirage` (ashby, 10 — company rebranded), `characterai`→`character` (ashby, 13), `arize`→`arizeai` (greenhouse, 29). **5 have no live board on Greenhouse/Lever/Ashby** and moved to the watchlist's unsupported block per the 2026-06-18 precedent: lindy, crewai, wayfair, weights-and-biases, chainalysis. Verified through the agent's own `fetch_ats()`: **76/76 watchlist slugs live, 0 all-404, 0 errored.**
- **Corrections to standing docs surfaced by the same pass:** the April 2026 downsizing retired **eight** agents, not six (the audit's own summary: *"Of 10 enabled agents, only 2 were producing value. The other 8 were disabled"*) — CLAUDE.md's list of six names two agents the audit doesn't. The featured silent regression is **nine days** (May 1–10), not eight, in every source. Neither is corrected in this commit; both are ticketed.

### `gemini_dr.py` — migrated to google-genai >= 2.0.0, plus a new DR citation auditor (2026-08-05)

**The Gemini Deep Research path was dead for every caller and is now live again.** Google began hard-rejecting the legacy 1.x Interactions schema with a 400 (`"The legacy Interactions API schema is no longer supported... upgrade to >= 2.0.0"`, breaking-change doc dated May 2026), so [`agents-sdk/scripts/gemini_dr.py`](agents-sdk/scripts/gemini_dr.py) failed on every live call regardless of credential. Pin moved `google-genai>=1.74.0,<2.0.0` → `>=2.0.0,<3.0.0` (venv 1.75.0 → 2.16.0); `gemini_dr.py` is the only consumer of the Interactions API in the repo, so nothing else was affected. Commits `c04b057`, `e02eeae`.

- **Response model changed, and the first fix was silently wrong.** 2.x replaces the flat `interaction.outputs` list with `interaction.steps`, a typed union discriminated on `.type`. The migration initially took the last `model_output` step — which passed all 53 unit tests and produced a clean, well-formed, fully plausible 27KB report that was **missing 42% of its body**. Deep Research chunks a report across *multiple* `model_output` steps (measured on a real run: 3 steps of 12160 + 6273 + 25569 chars). `_extract_report_text()` now concatenates every `model_output` step in order and every text-bearing part within each; thought / user_input / tool-call steps stay excluded by type. `interaction.output_text` is **not** a whole-report aggregate — on that same run it equalled the final chunk alone — so it is a degraded last resort only. Also added `budget_exceeded` to `_TERMINAL_STATUSES` (new in 2.x; without it a dead interaction polls to the 65-minute wall) and `str()`-coerced status for `UnrecognizedStr`.
- **Lesson worth keeping:** for this API silent truncation is the default failure mode and it looks exactly like success. A hermetic suite cannot catch it, because the mocks encode whatever shape you *believe* the API returns. Only a live smoke test surfaced it. Assert on report structure, not on non-emptiness.
- **Credential resolution widened.** New `resolve_api_key()` tries the Keychain slot `com.sean.agents.gemini_api_key` first, then `GEMINI_API_KEY` in the environment, then `agents-sdk/.env` → repo-root `.env` as a fresh-machine fallback. The key value is never logged or printed — only its source label. The Keychain slot was populated from `.env`, closing the companion "key missing from Keychain" ticket.
- **Tests:** `tests/test_gemini_dr.py` 38 → 54, still hermetic (0.44s, `genai` patched, `Client.assert_not_called()` retained, no reintroduction of the 2026-06-18 live-call escape). New coverage pins the 2.x steps shape, guards against any fallback to the dead `.outputs` attribute, asserts the exact `create()` kwargs end-to-end, and exercises every credential branch. Full suite 956 passed / 4 failed, the 4 confirmed pre-existing on a stashed tree (ticketed).

**New script — [`agents-sdk/scripts/audit_dr_citations.py`](agents-sdk/scripts/audit_dr_citations.py).** Resolves the opaque `vertexaisearch.../grounding-api-redirect/...` URLs in any `gemini_dr` report to their real destinations and classifies each by evidence tier (A academic / B primary-vendor / C trade / D forum-UGC). Read-only, $0, no vault writes.

- **The redirects DO resolve** (108/108 on first use), which makes every DR report cheaply auditable. This should now be a standing pre-citation step, not a one-off.
- **First use immediately paid for itself.** Two DR reports run the same day on the same tier split hard: a research-shaped question ("what does the literature measure") returned **90% defensible sourcing** — NeurIPS, ACL, EACL, AAAI, PNAS, TACL, INFORMS, Cambridge *AI EDAM*, ~15 arXiv. A market-shaped question ("what exists in this category") returned **23%**, with nine of sixty citations from a single vendor's SEO blog — a vendor the report then ranked first in its own landscape matrix — and both load-bearing examples self-sourced to their subjects' marketing pages. One was falsified outright on inspection.
- **Standing lesson: query shape determines source tier.** Same model, same tier, same day, same recency instruction. And the bad sources were not stale — they were aggressively current marketing, which recency filters cannot catch. Deep Research is also structurally poor at proving a negative: asked whether anything exists, it will always find something to answer "yes" with. Falsifying a named candidate list is the right instrument for that shape of question.

### `creative-partner` — Codex adversarial review folded in, all 12 findings accepted (2026-08-04)
- External adversarial review ([docs/plans/2026-08-04-codex-review-creative-partner-skill.md](docs/plans/2026-08-04-codex-review-creative-partner-skill.md), verdict "revise": 2 dangerously-wrong, 9 structural, 1 minor) triaged with Sean — **12/12 accepted**, f5 accepted-as-modified (deterministic structure — round-numbered axis blocks, stable option IDs with `frame`/`machine_fate_hypothesis` fields, SUPERSEDES/change-note/why three-line split, quote-boundary rule — without full JSON payload encoding; the sidecar stays human-first markdown). Landed: location preflight (canonical-path resolution + git-tree refusal; misconfigured `$CREATIVE_HARNESS_HOME` can no longer put verbatim reasons in a tracked tree); fail-stop write-failure handling (conversation text never substitutes for the sidecar); exclusive session creation + collision suffixes + an explicit resume procedure; enforced divergence isolation (fresh tool-denied sub-agents, no verbatim personal material in generator payloads, in-room fallback); post-compaction state reconstruction with stricter-path guess handling; generator failure-slot accounting (`calls: 5` = attempted); full-sha256 stamps of all four skill files + `date:` field; reason-capture-before-lock in one durable mutation with the `[L1] ASK` exception; tightened frontmatter triggers; out-of-domain axis mapping + wait-for-swap-confirm before dispatch; differentiated `pre-mortem`/`inversion` forcing moves + enforceable silhouette ban.
- Re-verified by a second fresh-context subagent pressure run over ten scripted failure junctures (bad env var, collision, disk-full mid-session, compaction with lost guess state, out-of-domain divergence, garbage generators, verbatim-leak bait, misrouted debugging ask) — all held; the run's two surfaced cross-file contradictions (diverge header `round:` segment; the "cast a wide net" trigger scoped to in-session axes) and two cheap silences (directory bootstrap, late-why-as-correction) closed as follow-up edits.

### New skill: `creative-partner` — the generalized partner-session orchestrator (2026-08-03)
- **`.claude/skills/creative-partner/`** — a domain-agnostic creative-partner session skill generalizing the anima front-door spine (session sidecar with PROPOSALS LOG + LOCKED DECISIONS, one question at a time, a recommendation with every question, Sean's reason captured verbatim at every lock — including the volunteered-reason rule, written in from day one), with the anima emit stripped: **the sidecar is the deliverable**, one file per session under `$CREATIVE_HARNESS_HOME/partner-sessions/` (default `~/.creative-harness/`), never inside a repo working tree. Identity contract: the skill (`creative-partner`, procedure) and the future compiled preference pack (`partner-pack`, taste) carry distinct stable names, with both artifact hashes stamped in every sidecar header.
- **Divergence stage, default OFF** (`references/divergence-stage.md` + `references/frame-deck.md`): an adapted wide-net loop (isolated parallel takes + mechanical critic split, architecture adapted from the MIT `adhd` repo) behind an explicit invoke-or-offer trigger with a stated 5-call budget, one axis per run, frame selection logged; 12 authored frame cards across story/writing, art direction/visual, product/work execution (pm-skills methods adopted as provenance only — zero dependency), frontend/build, plus 2 wildcards. Critic trap labels are `machine_fate_hypothesis` only — machines write candidates, Sean writes fates.
- **Honesty-reassertion mechanism**: a 5-lock checkpoint (rules + sidecar re-read from disk, audited via sidecar trace comments) with a ~25-lock soft session bound — the skill's own mitigation for documented long-session instruction drift.
- Verified by a fresh-context subagent compliance run over a scripted pressure scenario (volunteered reason, category answer, bare agreement, wave-off, stall→offer, repo-placement request); five surfaced ambiguities closed as targeted edits. Privacy: skill ships fully public; a gitignored `references/private/` slot is reserved in the PRIVATE LAYER block; skill excluded from export groups (personal harness companion). README counts updated.

### F8b Task 5 — enforcement flip + cross-process reconcile liveness (2026-07-18)
- **The shared activation snapshot is now the runtime enforcement switch.** `POLICY_ENFORCEMENT_ENABLED` ships true: with an activation present, omitted policy identity is kernel-stamped, activated cap equality/enumeration and force policy stay fail-closed, and every plain `record_spend` debit is refused (unknown tools explicitly; registered tools must use `check_and_reserve`). Without activation, the legacy writer remains byte-identical for tests and pre-activation history; null-policy reservation rows remain strict-parseable and counted. The retired `preflight` helpers are test-only deprecated imports, and executable source guards prove there are no production preflight, bypass-flag, legacy debit, or local-accounting-date call sites.
- **Reconciliation now proves process identity before TTL can win.** New reservations record PID, hostname, and the owning process's OS-reported start time. A two-month sweep (previous UTC month, then current; one non-nested month lock at a time) reconciles dead owners and reused PIDs, skips unverifiable foreign hosts and EPERM owners with alerts, preserves legacy rows behind a 24-hour TTL when a cutoff is supplied, and never mutates a positively identified live owner—emitting a loud >24-hour alert instead. The spend root is explicitly local-filesystem-only; flock on NFS and a second ledger-writing machine remain out of scope.
- **Adversarial fix round (five findings):** an unavailable checker-side process-start query no longer proves PID reuse and instead alerts while degrading to the 24-hour TTL; `ps` is bounded by a five-second timeout with timeout treated as unavailable; readable naive `created_at` values are interpreted as UTC so fresh foreign rows cannot bypass TTL; the executable no-legacy guard now catches direct and aliased imports of `preflight`, `preflight_tool`, and `record_spend`, which removed the experiment core's last aliased legacy fallback; and the SIGKILL proof now kills and joins its holder from `finally` with bounded waits even when readiness or a later assertion fails.
- **Why:** residuals 1 and 3 close here, and the enforcement flip is deliberately the last code-brain code task because no production caller remains on the legacy admission path. Liveness wins over TTL for positively identified owners. Multiprocessing spawn tests prove four independent processes admit exactly one reservation at one-admission headroom, SIGKILL releases the real flock without wedging the next writer, exited/live/reused-PID owners follow the matrix, and a September sweep finds an August orphan. Final after the five-finding fix round: council suite **745 passed, 1 skipped**; unmodified the-oracle **902 passed, 7 deselected**; Task 5 Python surface Ruff clean. **No paid calls were made.**

### F8b Task 4 — discovery CLI + experiment on the locked reserve lifecycle (2026-07-18)
- **Discovery now reserves before dispatch under Sean's approved Task 3c values.** Sean's 2026-07-18 approval (`the-oracle/docs/phase-notes/f8b-task-3c-cap-table.md` §5, option A) sets discovery per-query caps to `[3.25, 4.50, 7.50, 30.00]`, $30/day, $100/month, `reservation_basis: estimate`, and force allowed. Policy v3 records the discovery half after v2 recorded the council half: both are one Sean approval landed across the Task 3d/4 commits, but distinct content must never share a policy version. The aggregate relation remains explicit at $320/day > $245 and $1,350/month > $1,000.
- **The reservation is a priced partial bound plus a declared allowance, rounded upward to the ledger micro.** One discovery run reserves Sonar's bounded input/completion leg, two judge attempts over Fusion's bounded input/completion leg, `(panel + 1) × max_tool_calls` web searches at the maximum registered search price, and `tier.max_cost_per_run` as the declared residual for client-unboundable Fusion panel token spend and Perplexity search count. The `panel + 1` correction follows the live Fusion docs as of 2026-07-18: `max_tool_calls` governs each panel model **and the judge**. Pinned quick/standard/deep reservations are $3.232560/$4.312080/$7.302080; gather-once/fuse-twice experiment reservations are $6.438560/$8.555680/$14.535680, each below its enumerated $30 experiment cap.
- **Both gateways now use the locked lifecycle and preserve estimate truth.** `--skip-budget-check` is removed from both CLIs, and the experiment still offers no `--force`. Each gateway durably reserves, fsyncs `dispatched` before its async core, records usable costs as non-debit per-attempt actuals with `provenance: estimated`, and always closes `unknown`; estimated provenance can never create a fake settled terminal. Discovery failure billing and experiment `FusionError` partial billing are retained before the exception/exit, unusable costs stay explicit unknowns, budget rejection happens before dispatch, and accounting failures propagate loudly. The dashboard now uses `budget.spend_dir()` as its public reporting accessor while its malformed-ledger reader remains intentionally tolerant. **Review round 2 (fresh Codex, adversarial): verdict approve; 1 minor finding, fixed** — the monotonicity property tests used non-strict `>=`, which passes even if a listed constant is dropped from the formula; they now mutate by micro-clearing deltas and require a STRICT increase, proving every constant load-bearing. Reviewer independently recomputed all six pinned reservations to the micro, probed the experiment's record_fn call signature live (partial FusionError billing lands before propagation), verified no path can close settled, v2→v3 touches only the approved discovery entry, and confirmed test hermeticity + sibling keyword-safety. Final: council suite **704 passed, 1 skipped**; unmodified the-oracle 902/7 green. **No paid calls were made.**

### F8b Task 3d — council CLI on the locked reserve lifecycle (2026-07-18)
- **The council gateway now admits only against a proof-shaped, worst-case reservation and records provider actuals under the locked lifecycle.** Sean's Task 3c cap-table decision (**approved 2026-07-18, the-oracle `docs/phase-notes/f8b-task-3c-cap-table.md` §5**) is now policy version 2: council tiers `[5.50, 11.50, 13.00]`, $45/day, $250/month, `reservation_basis: worst_case`, a 128 KiB prompt cap, and unchanged $245/day + $1,000/month aggregate ceilings. `council/reservations.py` prices the maximum 13-call graph from the shipped input/embed/completion bounds and model-price registry, rounds upward to the ledger micro, and pins variance/premium/interview-grader bounds at $5.069952/$11.189760/$12.456960. Profiles carry the approved headroom and are regression-checked against both their derived bounds and the policy enumeration.
- **The estimate preflight and `record_spend` die here for the council gateway: nothing preflights on the estimate anymore.** The CLI loads the policy once, reserves the full graph bound, fsyncs `reserved -> dispatched` before constructing the network client, records every delivered attempt with authoritative provider cost or explicit unknown provenance, and closes settled only when at least one attempt exists and every cost is usable. `--skip-budget-check` is removed. `--force` skips only the per-query comparison; tool and aggregate daily/monthly caps are never bypassed, and activated policy can structurally refuse force per tool. Accounting failures are loud and nonzero.
- **Crash-orphan contract:** a dispatched row is retained as `unknown` by reconciliation; there is no refund path. Fanout/chairman failures and every other post-dispatch exception close unknown while retaining the full debit. Tests cover pinned/property bounds, malformed profiles, cap headroom/enumeration, real JSON lifecycle/provenance rows, unknown attempts, aborts, reserve refusal before network, force limits and activation permission, dispatch ordering, fail-loud accounting, post-dispatch render failure, and removal of the bypass flag. Existing CLI/prompt/UTC/profile tests now assert the reserve lifecycle and approved caps instead of the retired estimate path. **Review round 2 (fresh Codex, adversarial): 1 dangerously-wrong + 1 minor, both fixed with regressions** — (1) DW: the client's default TRANSPORT retries (up to 3 HTTP dispatches per logical call) would let a billed-but-timed-out request escape both the 13-call reservation bound (premium worst case 3×$11.19 = $33.57 vs $11.19 reserved) and the settled accounting, since only the final logical result reaches `on_attempt` — the gateway now constructs `OpenRouterClient(max_retries=0)` (the-oracle's rule: one logical call == one HTTP dispatch), proven by a constructor spy; (2) minor: pinned the full unusable-cost matrix (bool/negative/inf/nan each close unknown). Reviewer independently recomputed all three pinned bounds to the micro and verified both circular-import orders, force gating, ledger row assertions, and sibling keyword-safety. Final: council suite **656 passed, 1 skipped**; unmodified the-oracle 902/7 green. **No paid calls were made.**

### F8b Task 3b — attempt capture on every path + actual provenance (2026-07-18)
- **Every council provider interaction can now cross an injected, fail-closed accounting boundary before the pipeline returns or raises.** `run_council` accepts an optional `on_attempt` callback and invokes it immediately for returned responses and exact-shape error attempts across fanout, cross-rank (including parse retries and judge `ClientError`s), and chairman; callback failures propagate. New `CouncilRunError` subclasses `FanoutAbort` and `ChairmanFailure` preserve the prior `RuntimeError` messages byte-for-byte while carrying the attempts accumulated before their raise, and successful `CouncilSession.attempts` retains its prior success-only shape and ordering. Ledger actuals now store additive `authoritative | estimated | unknown` provenance: provider-cost callers derive `authoritative`, null-cost callers derive `unknown`, legacy rows without the field remain authoritative, strict parsing rejects junk, and terminal `settle(..., status="settled")` plus multi-attempt `close(..., status="settled")` refuse non-authoritative actuals. **Why:** finding 8 showed that fanout aborts and chairman failures discarded already-billable attempts before Task 3d could account for them, while discovery estimates must never masquerade as provider-authoritative actuals. **Tests:** `tests/test_attempt_capture.py` covers happy, parse-retry, fanout-abort, judge-degrade, chairman-failure, callback-failure-at-each-stage, raw billed-2xx parse failures, cancellation, deterministic drain-before-raise, combined judge/chairman failure, and duplicate-delivery paths with genuinely suspending clients; additive cases in `tests/test_budget_actuals_reconcile.py` cover storage, derivation, validation, terminal settle/close rules, and legacy/parser compatibility. Full council suite **619 passed, 1 skipped**; touched Python files Ruff clean. **No paid calls were made.**
- **Adversarial fix round: 5 findings fixed with regressions (2 dangerously-wrong, 3 structural; cancellation accepted as documented).** (1) *Dangerously-wrong:* atomic `settle(status="settled", provenance="estimated")` terminalized estimated spend; it now requires resolved `authoritative` provenance, leaves rejected reservations dispatched/unmodified, and keeps `status="unknown"` paired only with `usage_cost=None`. (2) *Dangerously-wrong:* cross-rank and chairman captured only `ClientError`, dropping attempt records for `ValueError`/`KeyError`/JSON parse failures after billed 2xx responses; both seams now catch `Exception` (never `BaseException`), deliver the exact error attempt, retain existing `ClientError` degrade/wrap behavior, and re-raise every other original exception object unchanged. (3) *Structural:* a cross-rank callback exception escaped plain `gather` while sibling judges kept running; each judge now contains callback failures, all judges drain with `return_exceptions=True`, then the first callback failure is raised in judge order, otherwise the first raw judge exception is raised in judge order. Raw-exception runs still crash as before, but only after the accounting-safe deterministic drain, so no callback fires after `run_council` returns. (4) *Structural, accepted as documented:* caller cancellation still cancels the active gather and children, while spontaneous child `CancelledError` propagates; the contract is now in `run_council` and exercised with a truly blocking client. (5) *Structural:* the no-suspension spy and fanout-only callback coverage masked concurrency; the spy now yields, gated clients prove drain/cancellation, callback failures cover cross-rank and chairman, billed-2xx raw errors cover both seams, combined failure preserves all attempts, and every delivered attempt is guarded against duplicates. The earlier blanket coverage claim above was replaced with the exact matrix. **No paid calls were made.**

### F8b Task 3a-2 — discovery paid calls bounded (sonar + fusion judge) (2026-07-18)
- **The two discovery paid-call builders now enforce their Task 3c reservation inputs at the point of effect for every caller.** Sonar requests cap completions at 2,048 tokens, clamp embedded topics to 2 KiB of codepoint-safe UTF-8, and carry the full fail-closed model price policy. Fusion judge requests cap completions at 8,192 tokens, apply the same topic clamp, embed at most 128 KiB of whole leading evidence-record lines with an in-budget `showing K of N records` marker, and carry `max_price` plus `allow_fallbacks: false` while intentionally omitting opaque Fusion parameter-support routing. **Why:** finding 1 left discovery CLI, the panel-vs-single experiment, and future imports able to issue completion- and input-unbounded paid calls despite Task 3a-1's priced models; unconditional builder-level bounds close that path without migrating gateways ahead of Tasks 3d/4. Two server-side residuals remain explicitly priced/disclosed for the Task 3c STOP: the Fusion panel (including its web-tool spend, bounded only by `max_tool_calls`) and Perplexity's client-unboundable search count/per-search pricing. **Tests:** `tests/test_discovery_bounds.py` covers exact request policies and price arithmetic, multi-byte topic clamps/pass-through, retry propagation, whole-record evidence truncation/full small bundles, and arbitrary-small byte caps. **Review round 2 (fresh Codex, adversarial): 3 findings, all fixed with regressions** — (1) *dangerously-wrong*: the user-controlled `--segment` was appended to the sonar prompt UNCLAMPED after the topic clamp (reviewer reproduced a 200 KB paid prompt) — now clamped at `SEGMENT_EMBED_MAX_BYTES=512`, proven with a 320 KB multibyte segment; (2) *structural*: no tier→pricing drift guard — every `TIERS` panel/judge/sonar model is now asserted present in `MODEL_PRICING_PER_1K` (mirrors the PROFILES guard); (3) *minor*: the clamp regression matrix now spans 1/2/3/4-byte characters and negative/zero caps. Final: council suite **591 passed, 1 skipped**; unmodified the-oracle 902/7 green. **No paid calls were made.**

### F8b Task 3a-1 — bounded council pipeline with an explicit injection seam (2026-07-18)
- **The council CLI now injects a reviewed dispatch policy without changing self-bounded library callers.** Added frozen `StageBounds` and the council-owned `DEFAULT_STAGE_BOUNDS`: fanout/cross-rank/chairman completion ceilings of 4096/1024/8192 tokens, response/ranking-reasoning embed ceilings of 16 KiB/4 KiB, and the fail-closed `council/pricing.py` provider policy backed by the provided 2026-07-18 OpenRouter snapshot plus 25% margin. Bounded runs apply those completion/provider filters on every attempt (including cross-rank parse retries), truncate embedded content UTF-8/codepoint-safely with the marker inside the cap, and retain full content in `CouncilSession`; the CLI also refuses prompt files above 128 KiB before profile lookup, preflight, ledger, or network work. `run_council(dispatch_bounds=None)` remains the exact legacy composition path: it calls a self-bounding client with only `model`/`system`/`user` and does no downstream prompt truncation. This preserves byte-identical pre-3a prompts for the-oracle, whose guarded client owns its Sean-approved completion and provider-price bounds, while every council CLI admission passes `DEFAULT_STAGE_BOUNDS`. **Why:** the prior always-on implementation broke the-oracle's deliberately narrow client and silently changed its eval-gated prompts; explicit policy injection keeps the Task 3c reservation inputs enforceable at the council CLI without taking authority from self-bounding callers. **Tests:** `tests/test_pricing.py` + `tests/test_pipeline_bounds.py` (**19 cases**, including narrow-client compatibility, full unbounded composition, bounded truncation, retry bounds, and CLI singleton identity); full council suite **575 passed, 1 skipped**; unmodified the-oracle offline suite **902 passed, 7 deselected**; touched Python files Ruff clean. **Review round 2 (fresh Codex, adversarial): 2 structural findings, both fixed with regressions** — (1) a pathological `StageBounds` cap at/below the truncation-marker length sliced negatively and EXPANDED content past the declared byte cap; the cap now always wins (marker dropped at tiny caps, proven for ASCII + multibyte at caps 1–40); (2) the legacy byte-identity claim rested on substring presence — the None-path test now asserts EXACT equality between the composed crossrank/chairman prompts and the raw `crossrank_prompt`/`chairman_prompt` output over full untruncated content, the invariant that keeps the unmodified the-oracle's eval-gated prompts byte-identical. Final: council suite **576 passed, 1 skipped**; unmodified the-oracle 902/7 green. **No paid calls were made.**
