...
- **KNOWN LIMITATION:** two model IDs named in the design spec (`google/gemini-pro-latest`, `mistralai/mistral-medium-3.5`) are not valid on live OpenRouter as of 2026-06-20 — the mock test suite is unaffected, but a live run needs valid IDs; verified replacements are listed in [`tools/llm-council/council/discovery/FUSION_SCHEMA.md`](tools/llm-council/council/discovery/FUSION_SCHEMA.md).
