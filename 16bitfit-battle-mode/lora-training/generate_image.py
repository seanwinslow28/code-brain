...
def generate_image(
    ...
    # Save image as true PNG (Gemini often returns JPEG regardless of prompt)
    ...
    if fmt != "PNG" and output.suffix.lower() == ".png":
        try:
            ...
            print(f"Converted {fmt} → PNG and saved: {output} ({final_size_kb:.1f} KB)")
        except ImportError:
            print("Warning: Pillow not installed, saving raw JPEG with .png extension.", file=sys.stderr)
