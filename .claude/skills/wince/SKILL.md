...
Emit gate below hard-fails when THE ONE MOVE is thin, so a stage actually skipped is a run that
...
A literal `mode: words-only` inside the block breaks the rule in **Marking a field thin** above.
...
**The decisions gate does not relax, and the honest count is three.** It fails a run at more than
...
\* A refused or sanitized generation moves these counts in both directions: Push can deliver zero,
and a sanitized return burns a generation that never reaches the user. See "When the model refuses"
above.
...
there, which is what "reuse" means here. It never means skipping the generation. The one exception
is a generation that gets refused or comes back sanitized, which is the only case where Push spends
zero, and it's handled in "When the model refuses" above, not here.
