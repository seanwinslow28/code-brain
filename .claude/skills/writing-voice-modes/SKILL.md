...
- As the **register law** the post-draft gates consult (see Related Skills)
...
State, in one line before composing: **dial % + the audience that sets it** (choose via the Professional Dial below). If the ask or an upstream brief already pins the dial ("dialed to 60%", a per-piece override), honor the pin: restate it, don't re-decide it.
...
| **Screenwriting Cut-To** *(prose only just)* | Hard juxtaposition between stated intent and actual reality. Prose has exactly **one** instance and it uses literal screenplay notation outside a screenplay, so the juxtaposition travels and the notation does not. | Civilized statement → "*cut to*" → all-caps chaos | "I'll be taking it easy tonight" *cut to* "SHOVE THE BEER BONG UP MY ASS." |
...
| **Equation / Formula Defamiliarizer** *(short-form only)* | Render an abstract cultural claim as a mathematical or categorical equivalence; the formula does the argument the prose would have to over-explain. The attested instances are all tweet-shaped, so it belongs to short-form. | "[Abstract subject] = [Concrete absurdity]" or "[X] were [Y] with the amount they [Z]" | "In 2026, PM = You and Claude = Entire P&E department." / "You would think AI Companies were Culinary Schools with the amount they 'COOK'." |
...
- **Suppressed-by-default backstory topics.** Certain real personal backstory is omitted from Sean's writing by default. It keeps creeping into drafts as stakes, motivation, or a sympathy-adjacent sideways aside. Cut it. Do not use it as setup, motivation, or an ask, **not even once**, unless Sean explicitly asks for it in a specific piece. The work stands on its own. The topic list lives in `creative-studio/content-machine/do-not-promote.md` (local-only). (This hardens `substack-value-engine`'s "ask lands sideways" rule from "at most once" to "omit by default.")
...
`creative-studio/content-machine/reference-universe.md`: Sean's actual cultural library: deep wells, quote canon, place/era texture, deployment mechanics, and the deprecation list. References must be pulled from this library or from the piece's actual subject. **Never invent a generic reference.**

**Before shipping any Substack draft**, read:
`creative-studio/content-machine/cheese-bank/cheese-bank.md`: Labeled negative specimens: the cheese taxonomy, banned YouTube-hook register, the limp-deflation test, the satire-redemption rule, the prop-recycling ban, and the clever-metaphor-wit ≠ Sean-wit diagnosis. Check the draft against it.
...
`creative-studio/content-machine/do-not-promote.md`: The Do-Not-Promote topic list. (The old `references/calibration-notes.md`, with its interview findings, moved verbatim to the private pencil-and-prompt repo on 2026-09-25.)
...
- `script-writing`: Sean's other medium, and one of the two corpora every move is attested against. Screenwriting Cut-To crosses into prose only just: the juxtaposition travels, the notation does not.
