...
> **The author modes are gone (2026-08-24).** This file used to carry four author-tuned modes (Sedaris, Thompson, Kerouac, Vonnegut) and a 90/10 framing that made Sean's voice a blend. Twenty author-labelled techniques were tested against his 9,804-word consolidated corpus: twelve survived and are named here for what they do, four merged into moves that already existed, and three were deleted, one of them **contradicted** by his own published work. The modes, the 90/10 spine, the Complementary Technique Pairs and the Content-Type→Mode table went with them. Rulings: [#161](https://github.com/seanwinslow28/code-brain/issues/161) and [#175](https://github.com/seanwinslow28/code-brain/issues/175). Do not reintroduce an author mode; if a technique is worth having, attest it and give it a functional name.
...
order he gave them, with connective tissue between, and mistook that for obedience. His verdict:
*"you're just taking what I said and scattering them everywhere"* and *"What's the purpose of the
skill chains and the writing examples if the answers I give during the interview are just slapped
and scattered all over the page verbatim?"*

The corpus governs **how**, not **what**. It decides which moves land where, the sentence shape and
diction, what to **cut** (an interview says far more than a piece should carry), and what order the
...
Each field has a consumer: the dial is what Sean or a chain orchestrator carries forward at the handoff (transport across the chain is the invoker's job, not this skill's); the named move instances tell a later scrub which patterns were deliberate; the last two lines prove the sweeps actually ran. For micro-pieces (one paragraph or less), the Step 1 line doubles as the record — restate it under the text with any moves used. On a critique-routed revise, re-run Step 4 on the changed spans and re-emit the record.
...
*(A third example, a mode-switching demo, was cut with the modes on 2026-08-24. It also
closed by repeating "So it goes" straight, which this file's own Sustained Flatness
anti-pattern bans: a canonical refrain has to be inverted, not copied. These two are
illustrations only. The register anchor is `references/voice-samples.md`.)*

## Technique Moves

Thirteen moves that came out of the author-calibration layer and survived attestation
against Sean's own corpus ([#161](https://github.com/seanwinslow28/code-brain/issues/161), 2026-08-24).
They are named for what they do. The author labels they used to carry are gone, and the
per-move "was" mapping lives on that ticket rather than in this file — the file carries
the working vocabulary, the ticket carries the archaeology.

**No examples column, deliberately.** The attestation pass found that this file's
mode-era illustrations were largely machine-written, and in one case the invented example
prescribed a register Sean has rejected. Per-move evidence is in the corpus and on the
ticket. Write from the mechanic; calibrate against `references/voice-samples.md`.
...
| **Sensory Cascade** *(corrected)* | **Domestic** sensory: kitchen sounds, cheap food, hangover fluids, the physical and the gross. **Not** physiological-thriller sensory (sweat, dilating pupils) — the corpus has zero instances of that, and it is a register Sean has rejected elsewhere. | A run of concrete bodily detail before the abstraction lands |
...
| **Flat Collision** | State a technical capability flatly, collide it with an irreducible human one. The best-attested move in the pass. | Flat capability sentence → flat human sentence, no connective |
...
| **Anaphoric Stack** *(discovered)* | Repeat an opening structure across consecutive clauses or sentences. Four shapes: **escalating**, **compounding**, **negating**, and plain repetition. Nobody had named this, so nothing in the file could ask for it — and two of the three moves deleted in the same pass turned out to be mis-descriptions of it. | Same opening words, N times, N ≥ 3 |

**Deleted in the same pass, do not reintroduce.** *Escalation loop* (a four-stage machine that appears nowhere in the corpus), *polysyndeton* (every candidate passage turned out to be Anaphoric Stack or plain listing), and *affirmative constraint* — "say what IS, not what isn't" — which was not merely unattested but **contradicted**: Sean builds with negation constantly, and the rule would have cut the closing line of the published Start Here page.
...
| **Sensory Before Numbers** | Smells, sounds, images FIRST. Numbers confirm and anchor. Joke-form cousin (script-confirmed): **precision-as-punchline** — an exact number, date, or price IS the punchline with no sensory preamble ("Fifty-One." listeners · "835 tickets" · "Scorpion: Ten Baht"). | Sensory paragraph → exact-number sentence on its own line | The sensory half runs **domestic**, per the Sensory Cascade correction — a physical-comedy buildup, then the exact number as its own line. The machine-written physiological-thriller illustration that used to sit here ("nostrils flare, pupils dilate") was cut on 2026-08-24: it prescribed the one sensory register the corpus contradicts. Live instances in `references/voice-samples.md`. Joke-form: "Fifty-One." · "835 tickets" · "Scorpion: Ten Baht" |
...
| **Screenwriting Cut-To** *(contested)* | Hard juxtaposition between stated intent and actual reality. Confirmed at origin in the scripts and confirmed in prose, but prose has exactly **one** instance and it uses literal screenplay notation outside a screenplay. Sent to the per-medium licensing audit ([#162](https://github.com/seanwinslow28/code-brain/issues/162)) as a contested cell rather than a clean row. | Civilized statement → "*cut to*" → all-caps chaos | "I'll be taking it easy tonight" *cut to* "SHOVE THE BEER BONG UP MY ASS." |
...
| **Self-Deprecation as Structure** | Be the biggest fool in the room first, then earn the right to observe everyone else. Absorbed two merged mechanics 2026-08-24: **self-implication** (criticize the system, but be the primary victim of your own incompetence within it) and **the triple position** (participate, observe, critique — earn observation by participating, earn critique by observing precisely). Both are postures this move already carried; House Style states the rule a third time. | Open with own incompetence → pivot to broader pattern | "I am a person who deploys to production at 11:47 PM" earns the right to critique every engineer who does the same. |
| **Blunt-Literal Description** | Strip the name; keep the concrete function, described flat. Blunt, never precious — do not soften the noun into a euphemism. **Restricted to non-tool objects** (a ferry, a TV show, a place, a ritual). Anything Sean built routes to **Tool-as-Character** instead: he does not defamiliarize his own tools, he casts them. Narrowed 2026-08-24 — the row's two former illustrations were machine-written *and* were both tools, which is what manufactured an apparent conflict with Tool-as-Character. Scoping error, not a contradiction; both moves come out clean. | "a [plain noun] that [absurdly literal function]", stated without comment | The ferry: "this floating death trap. Back and forth. Going nowhere fast." · The Masked Singer, never named: "Some sort of show where a celebrity sings and dances in a suit. Other celebrities have to guess who it is." |
...
| **Equation / Formula Defamiliarizer** *(short-form only)* | Render an abstract cultural claim as a mathematical or categorical equivalence; the formula does the argument the prose would have to over-explain. **Restricted 2026-08-24**: zero instances in 8,663 script lines, and the two prose instances are from one calibration round on one day, both tweet-shaped. Kept because it is plainly his and not a machine invention, but it carries the short-form restriction into the licensing matrix rather than posing as general-purpose. | "[Abstract subject] = [Concrete absurdity]" or "[X] were [Y] with the amount they [Z]" | "In 2026, PM = You and Claude = Entire P&E department." / "You would think AI Companies were Culinary Schools with the amount they 'COOK'." |
| **Inverted Refrain** | Take a canonical refrain and flip its vector: same cadence, opposite meaning. Inversion counts as invention; copying does not. Absorbed the refrain mechanic 2026-08-24: a chosen phrase deployed after each instance accumulates meaning through repetition, and the final instance shifts tense or form to resolve ("I began again" → "I have begun"). | Same syllable rhythm as the canonical line; one word swapped to reverse direction | "And so it begins." (inverting "And so it goes": resignation → anticipation) |
...
| **Rhetorical Catechism** | Ask yourself a question and answer it immediately, in your own voice. Promoted off the watch list 2026-08-24: parked since June on one borderline script instance, the prose corpus supplied the full triple across two independent documents. Same story as Sensory Before Numbers — a prose move the scripts had no room for. | Question sentence → immediate answer sentence, no pause; the triple is question, answer, consequence |

Nine of these rows were mined from the 11-screenplay corpus (2026-06-04); per-move evidence with 2+ script instances each lives in `drafts/2026-06-04-script-mining-report.md`. All 23 have now been attested against **two** independent corpora roughly a decade apart — the scripts in June, Sean's prose in August ([#175](https://github.com/seanwinslow28/code-brain/issues/175)). **No move was contradicted in either corpus.** That is the finding: moves derived from Sean's own writing survived a second corpus intact; the moves derived from author calibration did not. Still on watch (insufficient instances): Post-Chaos Inventory Shot.
...
Five rows were renamed on 2026-08-24 when the author modes were deleted. The content is
unchanged — a rename only, per the ruling; what each one catches was never about an author.
...
| **Critique Without Self-Implication** | Chaos without factual anchoring. Criticism without self-implication. Using first-person as an excuse to skip the reporting. Gonzo without journalism is just a tantrum. |
| **Sustained Flatness** | Sustained flatness. Sean needs runway. *Copying* "So it goes" verbatim instead of inventing your own refrain (inversion counts as invention, "And so it begins" earns the right to echo because it flips the vector; "So it goes" repeated as-is does not). Short sentences because you have nothing to say. |
...
| **Lexical Repetition (incl. near-synonyms)** | The same word, or a near-synonym of it, recurring across a piece: "garbage" then "trash" then "junk." Sean's rule (Round 7): one instance of a concept-word per piece; at most one early and a *different* one late if truly needed, never a third. Repeating a word OR its synonym reads as thin vocabulary. Pick the single strongest word for the idea; vary or cut the rest. Generalizes "Bad Sean" (over-using one sensory detail) from props to lexis. |
| **Interior-Translation Tell (VoicePrint R1)** | The construction family `which is <someone>'s way of saying <punchline>` and its variants ("a polite way of saying..."). Traced to the Sedaris calibration; banned on sight. It claims access to another party's interior and then performs the translation — the opposite posture from Sean's under-reaction and flat-report comedy. |
| **Forced-Memory Cutaway (the Family Guy problem)** | Personal-history material from the corpus jammed into a sentence it has nothing to do with. The existing Reference Gorging row covers POP-CULTURE references; this is the same failure using **Sean's own life**, and it is the one that actually burned him. His description: the old skill would write "spending time playing basketball with my friends on Buffalo Street" into an unrelated technical passage. *"My personal experience was there, but it still didn't make any sense with the stories and topics being written."* The mechanic is the Family Guy cutaway: a character is in a situation, pulls a random unrelated memory, and the show cuts away to it. Test: does this detail belong to THIS moment, or did it arrive because it was in the memory bank? Ratified run #2, 2026-08-28. |
| **Doubled Image (not doubled word)** | Two different words carrying the same physical image inside a short span. The Lexical Repetition row above catches a repeated WORD or near-synonym; it does not catch this, because the words are unrelated. Sean cut "spewed" because it was followed by "spilling your guts onto a keyboard": *"We get the picture."* Test the images, not just the vocabulary. Ratified run #2, 2026-08-28. |
...
- [ ] No author mode reintroduced, in the prose or in the record
