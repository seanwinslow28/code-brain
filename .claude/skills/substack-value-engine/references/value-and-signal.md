...
- Story crest: the nightly critic had silently failed for a week.
...
- **Confident** self-deprecation punches at a *specific past decision* ("I shipped the pixel mascot before I had per-view references; predictably, it drifted"). That is a blameless post-mortem: seniority.
...
This rule **defers to** `writing-voice-modes`, which owns the anti-pattern as a tonal rule. This skill enforces it as a structural gate (where the ask may appear, and that the piece ends on work/lesson).

## Worked gate examples

**PASS.** Itch: "my nightly vault-critic logged antigravity 5/5 fail for a week and the meta-agent missed it." Solution: "root-caused to gemini-CLI MCP startup hanging past the 120s timeout; fixed with `--allowed-mcp-server-names __none__`; added per-CLI error persistence." Transfer: "the reader can now spot a silently-tolerated failure state in their own agent fleet and add the one diagnostic that surfaces it." All three concrete and checkable → PASS.
...
**BLOCK.** Itch and Solution fine, but Transfer is "readers will understand agent observability better." Vague, no concrete capability. → BLOCK at the Transfer slot; sharpen to a specific thing the reader can DO.
