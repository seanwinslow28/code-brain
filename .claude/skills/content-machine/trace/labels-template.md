...
- **`verdict`** — `pass` or `fail`, nothing between; or `defer` (since kit 0.7.0, DESIGN.md §15), a recorded "come back to this" that is not a verdict: the row still waits, never reveals a blind pair, and is never counted as labeled. "The other picks can use a little work" is a fail with a critique. An empty cell is a row still waiting; the checker counts it and the viewer's counter shows it.
