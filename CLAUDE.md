...
| Knowledge loop (consumer) | SessionStart hook injects `vault/knowledge/index.md` as additionalContext (15K cap, 5s timeout). **Renders compact slug rows, not the file's wikilinks** (#202): the same budget now carries **all 218 concepts + ~129 connections** instead of 190 concepts and zero. A truncated section is selected by `updated:`/`created:` frontmatter, never mtime (a sync rewrites every mtime at once), and always states how many rows it dropped. `index.md` itself is untouched so Obsidian still navigates by wikilink. | [`.claude/hooks/session-start-inject-index.sh`](.claude/hooks/session-start-inject-index.sh) |
