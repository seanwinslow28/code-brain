...
9. **Privacy layer (2026-06-10)**: This is a public repo. Personal and employer-confidential material lives on local disk only, gitignored under the `PRIVATE LAYER` block in [.gitignore](.gitignore) — the job-hunt home (`vault/20_projects/prj-job-hunt-2026/`, incl. `source-material/` and its own private `tickets.md`), operating models (`vault/05_atlas/operating-models/`), timeline notes (`vault/10_timeline/`), skill reference profiles (`.claude/skills/{personal-finance,life-admin}/references/`, `.claude/skills/writing-voice-modes/{references,drafts}/`), and The Block archives (`the-block/`, granola notes, media-team-ideas, `vault/60_archive/operating-models-the-block-2026-05/`). The Pencil & Prompt writing brain (corpus, voice samples, lessons ledger, cheese bank, transcripts, Oracle reports) moved on 2026-09-25 to its own private repo, `pencil-and-prompt` ([#308](https://github.com/seanwinslow28/code-brain/issues/308)). Its old paths under `creative-studio/content-machine/` are git-ignored symlinks into that repo on the MacBook Pro, and their ignore rules stay. **Never `git add` these paths, never weaken these ignore rules, and never write real income/medical/contact/employer data into tracked files** — public skills point at their local-only reference files instead. History was scrubbed via git filter-repo (see [docs/2026-06-10-code-brain-recruiter-readiness-audit-plan.md](docs/2026-06-10-code-brain-recruiter-readiness-audit-plan.md)). **Agents that write into tracked paths must not describe private files either** (2026-09-03): `knowledge_lint.py` routes any finding that is SOUL-derived, or that names a file in a gitignored subtree, into `vault/health/private/` (ignored) and withholds it from the tracked report, keeping counts intact. `PRIVATE_VAULT_PREFIXES` mirrors the ignore rules below and a test fails if the two drift.
...
**Local TTS** — [`agents-sdk/scripts/doc_to_audio.py`](agents-sdk/scripts/doc_to_audio.py) renders vault markdown to verbatim narration via Kokoro-82M ONNX (Apache 2.0, $0/run on Apple Silicon). Single voice (default `af_heart`); structural markdown is flattened or spoken-cued; output at `vault/90_system/audio/<source-stem>.mp3`; CLI is idempotent on mtime. Fresh-machine setup: [`agents-sdk/scripts/install_tts_models.sh`](agents-sdk/scripts/install_tts_models.sh). Decision record: [`agents-sdk/docs/local-tts-decision-record.md`](agents-sdk/docs/local-tts-decision-record.md). Spotify handoff sketched at [`agents-sdk/docs/local-tts-spotify-handoff.md`](agents-sdk/docs/local-tts-spotify-handoff.md), deferred until 10+ clean pipeline runs. **First production consumer (2026-09-03, #239): the Content Oracle's weekly listening report** via `news_lane.py render` (the Oracle moved to the private `pencil-and-prompt` repo on 2026-09-25, #308, and reaches this script through `$CODE_BRAIN_DIR`) — 1,257 words rendered to 7:59 at 158 wpm on the first run.
...
| Claim-6 Drill (**default disabled; registration required**) | Registered day-of-month at 08:15 monthly | Deterministic ADR-12 runner; priority-2 Pushover receipt; drill-only JSONL; 08:45 meta-agent reconciliation | $0.00 |
...
| Substack-Drafter (**default disabled**) | Thursday 18:00 weekly (when `INSTALL_SUBSTACK_DRAFTER=1`) | HybridRouter (Qwen3-14B local → Sonnet 4.6 fallback); reads code-brain's frozen writing-voice-modes SKILL.md verbatim (the live copy moved to `pencil-and-prompt`, #308); 5-week voice rotation (sean/sedaris/kerouac/thompson/vonnegut) | $0–0.10/run (cap $0.10) |
...
Config: `agents-sdk/config.toml`, deep-merged under a gitignored `agents-sdk/config.local.toml` when present ([lib/config.py](agents-sdk/lib/config.py) `load_raw_config()`; see [config.local.toml.example](agents-sdk/config.local.toml.example)). Machine-specific deploy state — the claim-6 drill registration, whose `acknowledged_device` is a personal device name — belongs in the override, never the tracked file, so `config.toml` ships unarmed and committable. A malformed override raises rather than being skipped. `Config.local_config_path` records which override applied and the meta-agent prints it, so the merge is never invisible. Auth: `claude login` OAuth (no API key). For unattended launchd runs, prefer a long-lived `claude setup-token` token stored in Keychain (`pbpaste | python3 agents-sdk/lib/keychain.py set --stdin claude_code_oauth_token` — use the `--stdin` form for every secret: it keeps the value off argv, so no shell parse and no shell history, and `set` prints the stored length so a truncated paste is visible) — [lib/auth.py](agents-sdk/lib/auth.py)'s `resolve_oauth_token()` injects it as `CLAUDE_CODE_OAUTH_TOKEN`, so headless runs don't 401 when the interactive credential expires (root cause of the 2026-06-20 morning 401). Safety: max 30 turns, default $0.50/run cap (daily-driver morning bumped to $0.60 to absorb the operating-model artifact preamble). SDK version: `0.1.63` (pinned in `agents-sdk/pyproject.toml`). Morning schedule: daily-driver 08:30 writes the note, meta-agent 08:45 reports on it. Full docs: `docs/agents-sdk.md`.
