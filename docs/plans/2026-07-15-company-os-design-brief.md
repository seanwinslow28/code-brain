...
**The sharpened claim:** groundwork is the only Company OS that **compiles governance** — typed rules with triggers, evidence, owners, and appeals, derived from an ontology interview — instead of shipping templates.
...
4. **Shared Discipline (anti-rot).** The launch gate — *"the feature is not rolled out until the repository is updated"* — plus a freshness-check skill (files untouched >90 days), append-only `## Hindsight` blocks on decision-log entries (captures calibration, and answers the role-transition/offboarding problem), and the correction habit (add a correction the moment the agent errs — Anthropic's own CLAUDE.md practice per Boris Cherny). Memory posture (decided): **conventions only, no system** — decision logs, working-notes files, freshness metadata, all plain markdown; the README points to gbrain/Open Brain as complementary memory layers.
...
- **The constitution compiler** + enforcement ladder + adoption ladder + two-axis maturity + Shared Discipline conventions (§5.1–5.6)
...
- Re-interview / drift flow (regeneration merging with local edits; memory-provenance labels — observed / inferred / confirmed / superseded — for anything agents write back)
...
> Any company can point its coding agent at this repo and, within an afternoon, get a governed, multi-harness Company OS compiled from its own work ontology — typed rules with owners and appeals, skills with named owners and honest compatibility notes — judgment built in, not bolted on.
