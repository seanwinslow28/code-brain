...
| **Accountability** | which business process runs differently, and **who is accountable for proving it improved** ("employees save time" is disqualified) — including a **baseline captured before provisioning** (the freshest verified pain in the discovery run: *"If your organization doesn't measure... before/after AI adoption, you cannot tell whether your AI investment is creating value or destroying it"*) | ref-nate-implementation-architecture + discovery ledger pain #3 |
...
6. **Adoption ladder.** Hub-and-Spoke / Full Adoption / Agent-Delegation with the decision rule ("<half the team uses AI → Hub-and-Spoke; more → Full; 10+ with eng → add Agent-Delegation"), the 4-week rollout (seed rough **+ capture per-activity baselines** → one person → two habits → open with the 3-query demo), the captain model, and two-track review made concrete by the action taxonomy (track 1 = read-only/reversible; track 2 = external-side-effect/high-risk + product-strategy/architecture review). Adoption content also addresses **fear directly** — reskilling framing ("AI replaces tasks first, not people"), role-redesign guidance — because resistance is a trust problem training alone doesn't fix (discovery gap-fill leads, CIO change-management guide).
...
## Appendix E: fusion-discovery-council pain-point pass (2026-07-16, final pre-wayfinder gate)

Standard-tier pm-lens run on "Company OS and org-wide AI agent adoption for non-engineering teams" (segment: founders/COOs/ops-PM leaders). **5 verified pain points, 0 dropped, $1.94.** Full ledger with evidence URLs, whitespace map, and verbatim-verified web supplement (backstop exit 0): [`vault/20_projects/research/2026-07-16-company-os-org-adoption-pm-idea-ledger.md`](../../vault/20_projects/research/2026-07-16-company-os-org-adoption-pm-idea-ledger.md).

| Verified pain (score) | groundwork's answer | Status |
|---|---|---|
| #1 "Confident Guesser" — agents answer authoritatively without live context (46, 4 domains): *"Every team that struggled early was struggling with context, not with the model"* | The generated OS **is** the context layer: ontology folders + org-memory schema + anti-rot freshness give agents structured, current company context | Validated core thesis |
| #2 Reliability/trust gap blocks delegation (41): outputs need *"heavy manual checking"* | Action taxonomy + proposed-only tiers + evidence-required-with-output (Owner's Card) + work-package Proof row | Covered |
| #3 Weak rollout discipline, missing success metrics (36, **freshest** — 2026-07-07): can't tell if AI *"is creating value or destroying it"* | **Was a gap — fixed:** baseline capture pulled into V1 (ontology accountability field + rollout week 1) | Design changed |
| #4 Agents amplify existing mess/bad data (35): *"agents amplify the mess"* | The Describability Gate + substrate column refuse to automate the un-describable — the interview surfaces the mess *before* provisioning; this is groundwork's answer to the ledger's contradiction map (turnkey optimism vs. no-tool-fixes-broken-ops) | Covered, now stated explicitly |
| #5 Adoption becomes an internal engineering project (35): want *"turnkey, context-rich... rather than yet another tool that turns into an internal engineering project"* | The interview→generate afternoon path; honest scope: one git-capable AI-Ops maintainer, employees never touch git | Covered (honest tension documented) |

Second design change from the gap-fill leads: adoption content now addresses **fear/reskilling directly** (§5.6) — resistance is a trust problem, not a training problem. The compliance-gap leads (UDAAP exposure, audit-log requirements) reinforce the V2 Classify→Consent→Enforce pack's priority.
