...
3. **Pinned grounding answers** (zoom-out-and-think's Step 1 hard gate, (a)–(c) — use verbatim, do not re-derive). The draft grounding was tightened on 2026-07-04 against the field report before any baseline ran — the draft's "register intent leaks across transport/handoff boundaries" overclaim was narrowed to the field report's actual claim (gpt-image's across-edit identity hold is unproven — an open risk, not a confirmed failure):
   ...
   - **(b) Tried:** universal NB2 as the hardcoded default for every register (confirmed in `pipeline/registers.py`); per-register prompt nudges (e.g., `90s-nicktoon-grossout`'s gross-up-ratio correction after Sean rejected the first draft's constantly-grotesque output — see `registers/90s-nicktoon-grossout/research.md` §0); per-register look-spikes ratified case-by-case (the `primal-sketch-grit` go/no-go spike, the `90s-nicktoon-grossout` cross-engine spike — see field report §1 for `primal-sketch-grit`; see `registers/90s-nicktoon-grossout/research.md` §0, §4 for `90s-nicktoon-grossout` — field report §4/§6 still listed that register as pending look-spike as of 2026-07-04).
...
**Footnote (2026-07-04):** Citation pointers in (b) corrected 2026-07-04 after baseline generation; grounding-answer substance unchanged (both model runs consume identical text).
...
4. **Run prompt** (self-contained; copy verbatim into a fresh session — code-brain checked out at `93e5725`, branch `feat/fable5-campaign`):
   ...
   > - `.claude/skills/writing-humanity-pass/SKILL.md`
   >
   > Skill-audit's Step 1 is a hard gate that normally requires asking the user four grounding questions — (a) For, (b) Feeds, (c) Disappoints, (d) Wow — before scanning anything. For this run, the answers are pre-pinned: read `docs/plans/wwf5d/task-battery.md`, section "BT3 — Pinned grounding answers," restate your understanding of each back in your output, then proceed directly to the chain-level Seam Scan without asking the user anything. Read this battery doc from the working tree at its own commit (`1d52b9e` or later) — only the harness and five chain files named above are pinned at `93e5725`, via `git show 93e5725:<path>` if your working tree has moved past that commit.
   >
