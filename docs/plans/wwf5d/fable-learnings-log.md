...
| 2026-07-05 | R2/eye-check | Paired Opus±WWF5D on skill-audit(preserve-session). With-WWF5D caught a contract-contradiction the baseline PROTECTED (two vault anchors have opposite write disciplines; blanket "PATCH not PUT" is half-wrong) + named the write transport | Verify-the-world axis confounded (grounding named the targets → both verified); the lift showed in the sharper moves | Eye-check gate passed → Round 2 greenlit | strength |
| 2026-07-05 | R2/RT1 | preserve-session fix-spec, paired (shared findings → isolates spec-decidedness). Fable's spec caught TWO contract-contradictions Opus's OWN spec shipped: a 10-entry cap that *drops* the oldest (data loss in a zero-loss spec) + Open-Questions written-but-never-read (Opus protected resume-session, leaving the value unread at point-of-effect) | Fable also operationalized transport+MCP-fallback+FAILED-payload-dump + named the concrete repo tripwire (Edit `old_string` stale mid-session → one-retry bound). Opus+ = the empty-session edge Fable missed | "Hold the spec's own stated objective as an invariant across every sub-decision" → §6 self-consistency; paired-change at point-of-effect → §2.7/§2.3 | strength |
| 2026-07-05 | R2/RT2 (ceiling) | skill-audit(hooks-configuration), paired. Ceiling probe: Fable's BT1 existence-check blind spot did NOT recur — un-WWF5D'd, Fable caught 3 inert protection layers (dead-key firewalls + invented permissions-schema + 5000s-timeout unit) to Opus's 1, plus flagged the skill TEACHES a bypassable substring firewall | Opus+ real: env-var opt-out bypass + "don't use prompt-hooks for security" — 2 false-safety patterns Fable missed. §2.4-efficacy-for-Opus/Sonnet routes to Step 4 | Existence-check emerges for Fable on an enforcement-dense substrate; keep §2.4 as an explicit Opus/Sonnet aid | ceiling |
| 2026-07-05 | R2/RT2+RT3 (research) | Fable proactively fetched the LIVE upstream hooks docs (RT2 → missing hook types + `permissionDecision`, "a platform version behind") and ran the real shipped `audit_intent_spec` (RT3). BUT on RT3 Opus researched too (external best-practice) | Research-*presence* fired for BOTH → promptable, NOT Fable-unique (F3). The premium is what research grounds, not the trigger | Retire "proactive-research trigger" as Fable-unique; keep §1.3 "read the live external contract, not just the local one" | cheap-on-opus |
| 2026-07-05 | R2/RT3 | portfolio explainer spec, paired. Backbone converged hard (GSAP harness, WebGL-for-flagship, PE-floor = cheap-on-Opus). Fable premium = the honesty/evidence tail: run the REAL shipped auditor (not a simulated counter), play the REAL 12fps loop (caught the figure contradicts its project's Engine-Truth — asserts a shipping loop, never plays it), build-enforced provenance, self-validated with the real tool | Opus+ = bolder pure material spectacle (a detailed WebGL ink-bleed shader Fable deferred as over-reach) — a legitimate different audience bet, Sean's eye | "Run/embed the real artifact under a provenance check; a convincing simulation is a regression against honesty-based intent" → §5.2/§5.4 refinement | strength |
