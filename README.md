...
An open-source agentic engineering practitioner's toolkit — **124** skills, 13 subagents, **14** hooks, **18** autonomous SDK agents (9 currently running on launchd by default, 2 opt-in disabled-by-default, 1 manual-trigger), **3 primary domains**, an Obsidian vault, and a Claude Agent SDK runtime, all auto-loaded. Every component is in active use; every scheduled agent has a launchd schedule; every skill is a prompt and every prompt has a job. A local verbatim TTS pipeline (Kokoro-82M ONNX, $0/run on Apple Silicon) turns vault docs into MP3s for commute listening. If you've read Karpathy's "agentic engineering" framing and wondered what one looks like in the wild, this is one.
...
|--------|-----------|-----------------|
| **the-block/** | (private, gitignored) | **Archived 2026-05** — prior-employer material kept on local disk only, out of the public repo. |
...
| **vault/20_projects/prj-job-hunt-2026/** | (private, gitignored) | Active job-search ops run through this same system — job-feed pipeline, plans, trackers. Lives on local disk only; see **Privacy boundary** below. |
...
This is a real, daily-driven personal system — which means personal data flows through it. The repo draws a hard line: **the machinery is public, the operating data is private.** Job-search operations, daily/weekly notes, operating-model artifacts (SOUL/USER/HEARTBEAT), financial and life profiles, and prior-employer material all live on local disk in a gitignored private layer (see the `PRIVATE LAYER` block in [.gitignore](.gitignore) and rule 10 in [CLAUDE.md](CLAUDE.md)). Public skills that need personal data read it from local-only reference files at runtime. Git history was rewritten with `git filter-repo` to enforce the same boundary retroactively — the audit and phased execution plan is itself a public artifact: [docs/2026-06-10-code-brain-recruiter-readiness-audit-plan.md](docs/2026-06-10-code-brain-recruiter-readiness-audit-plan.md).
...
| Meta-Agent | 8:45 AM daily (v3.38.0 — was 8:35; now runs *after* daily-driver, eliminating the `Daily note exists: No` race) | Fleet self-monitoring — checks active-agent recency + infra health, writes fleet-status note that feeds Daily Driver |
