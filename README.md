...
An open-source agentic engineering practitioner's toolkit — **117** skills, 13 subagents, 13 hooks, **15** autonomous SDK agents (8 currently running), **3 primary domains**, an Obsidian vault, and a Claude Agent SDK runtime, all auto-loaded. Every component is in active use; every agent has a launchd schedule; every skill is a prompt and every prompt has a job. If you've read Karpathy's "agentic engineering" framing and wondered what one looks like in the wild, this is one.
...
### 117 Skills Across 12 Export Groups
