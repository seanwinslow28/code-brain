...
class TestArtifactPath:
    def test_path_composition(self, tmp_path: Path):
        p = artifact_path("creative-studio", "HEARTBEAT", tmp_path)
        assert p == tmp_path / "05_atlas/operating-models/creative-studio/HEARTBEAT.md"
...
class TestLoadArtifactHappyPath:
    def test_reads_confirmed_heartbeat_and_strips_frontmatter(self, tmp_artifacts: Path):
        body = load_artifact("creative-studio", "HEARTBEAT", tmp_artifacts)
        assert body is not None
        assert body.startswith("# HEARTBEAT — Creative Studio")
        assert "Block work through 3 PM" in body
    ...
    def test_distinct_bodies_per_domain(self, tmp_artifacts: Path):
        ...
        job = load_artifact("job-hunt-2026", "HEARTBEAT", tmp_artifacts)
        assert "Creative Studio" in creative
        assert "Life Systems" in life
        assert "Job Hunt 2026" in job

    def test_returns_body_without_stripping_when_opted_out(self, tmp_artifacts: Path):
        body = load_artifact(
            "creative-studio", "HEARTBEAT", tmp_artifacts, strip_frontmatter=False
        )
        assert body is not None
...
class TestLoadArtifactMissingFile:
    def test_missing_file_returns_none(self, tmp_path: Path):
        result = load_artifact("creative-studio", "HEARTBEAT", tmp_path)
...
class TestRequireConfirmed:
    ...
    def test_draft_returns_none_when_required(self, tmp_artifacts: Path):
        self._rewrite_status(tmp_artifacts, "creative-studio", "USER", "draft")
        result = load_artifact("creative-studio", "USER", tmp_artifacts)
        assert result is None

    def test_draft_returns_body_when_not_required(self, tmp_artifacts: Path):
        self._rewrite_status(tmp_artifacts, "creative-studio", "USER", "draft")
        result = load_artifact(
            "creative-studio", "USER", tmp_artifacts, require_confirmed=False
        )
        assert result is not None
        assert "USER — creative-studio" in result
...
class TestCaching:
    def test_cache_hit_does_not_reread(self, tmp_artifacts: Path, monkeypatch):
        first = load_artifact("creative-studio", "HEARTBEAT", tmp_artifacts)
        ...
        second = load_artifact("creative-studio", "HEARTBEAT", tmp_artifacts)
        assert second == first
        assert read_calls["count"] == 0  # cache hit, no re-read

    def test_cache_invalidates_on_mtime_change(self, tmp_artifacts: Path):
        path = artifact_path("creative-studio", "HEARTBEAT", tmp_artifacts)
        first = load_artifact("creative-studio", "HEARTBEAT", tmp_artifacts)
        ...
        new_body = (
            ...
            "status: confirmed\n"
            "---\n\n"
            "# HEARTBEAT — Creative Studio (revised)\n\n"
            "New content.\n"
        )
...
class TestLoadHeartbeats:
    ...
    def test_missing_domain_maps_to_none(self, tmp_artifacts: Path):
        ...
        missing = artifact_path("life-systems", "HEARTBEAT", tmp_artifacts)
        missing.unlink()
        result = load_heartbeats(tmp_artifacts)
        assert result["life-systems"] is None
        assert result["creative-studio"] is not None
        assert result["job-hunt-2026"] is not None
